...
`NEG-PATH-MUTATION-BOUNDED-INTERVAL-ANGLE-BOUNDS-INVERTED`もcanonical manifest外のartifact-contract parser候補です。同じaccepted controlの`e-hinge`最初のinterval angle bound `[0,pi/2]`でlower boundだけを`0`から`3pi/4`へ置換し、完全な1-path差分と、親boundの`invalid-bounds`（`lower bound must not exceed upper bound`）、親interval mapの`angle-outside-bound`（`interval bound for e-hinge must contain both adjacent knot angles`）という順序付き2 issueを再実行します。有限なinverted pairを`boundsByEdge`へ保持するためordering失敗後もliteral containment検査を行います。inner tuple／outer row長、interval cardinality、crease ID集合を保ち、finite、cardinality、mapの二次issueは出ません。これは保存されたbound orderingと隣接knot angleのliteral containmentに対する現parser境界だけであり、物理的angle schedule／boundの妥当性、保守的bound、kinematic feasibility、endpoint／物理経路semantics、representation選定、crease-map completeness、piecewise-polynomial、剛体性、面等長、ヒンジ幾何、certificate hash／真正性、contact、CCD、collision freedom、foldability、SupportProfile、ToleranceProfile、canonical family全体、科学的検証、global M0F `GO`を意味しません。
...
Worker測定は成功経路を各5回実行し、candidate-onlyのraw JSONLとhash-bound manifestを
