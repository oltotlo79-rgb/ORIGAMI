...
import { runNegPathBoundedFiniteAnglesCandidateBundleCli } from '../../m0f/neg-path-bounded-finite-angles-candidate-bundle-cli.js';
import {
  ...
  NEG_PATH_BOUNDED_FINITE_ANGLES_CASE_SPEC_V1,
  NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
  boundedFiniteAnglesJsonDifferencePaths,
  buildNegPathBoundedFiniteAnglesCandidateBundleV1,
  parseNegPathBoundedFiniteAnglesCandidateBundleLedgerV1,
  verifyNegPathBoundedFiniteAnglesCandidateBundleV1,
  writeNegPathBoundedFiniteAnglesCandidateBundleV1,
} from '../../m0f/neg-path-bounded-finite-angles-candidate-bundle.js';
import { parseFixtureManifest, sha256Prefixed } from '../../m0f/manifest.js';
import { stableStringify } from '../../m0f/stable-json.js';

const BUNDLE_DIRECTORY = resolve(NEG_PATH_BOUNDED_FINITE_ANGLES_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY);
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-bounded-finite-angles-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathBoundedFiniteAnglesCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-BOUNDED-FINITE-ANGLES candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathBoundedFiniteAnglesCandidateBundleV1>
  >;

  beforeAll(async () => {
    committedLedger = record(
      await jsonFile(
        join(BUNDLE_DIRECTORY, NEG_PATH_BOUNDED_FINITE_ANGLES_CANDIDATE_BUNDLE_LEDGER_FILENAME),
      ),
    );
    validate = new Ajv2020({ strict: true, allErrors: true }).compile(
      record(await jsonFile(SCHEMA_PATH)),
    );
    committedVerification = await verifyNegPathBoundedFiniteAnglesCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    expect(
      parseArtifactContractV1(NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1),
    ).toMatchObject({ ok: true });
    expect(NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one non-finite bounded angle and the sole parser oracle', () => {
    expect(
      boundedFiniteAnglesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_BOUNDED_FINITE_ANGLES_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_BOUNDED_FINITE_ANGLES_CASE_SPEC_V1.changedPaths);
    ...
    expect(angleArray(source)).toEqual([null, Math.PI / 2, Math.PI]);
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(await jsonFile(join(BUNDLE_DIRECTORY, 'path-bounded-finite-angles.json'))).toEqual(
      NEG_PATH_BOUNDED_FINITE_ANGLES_CASE_SPEC_V1.sourceDocument,
    );
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(parseNegPathBoundedFiniteAnglesCandidateBundleLedgerV1(committedLedger).ok).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted = parseNegPathBoundedFiniteAnglesCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathBoundedFiniteAnglesCandidateBundleLedgerV1(changed).ok).toBe(false);
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathBoundedFiniteAnglesCandidateBundleV1();
    expect(first.files).toEqual(buildNegPathBoundedFiniteAnglesCandidateBundleV1().files);
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-BOUNDED-FINITE-ANGLES',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathBoundedFiniteAnglesCandidateBundleCli(['--verify'], io)).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-bounded-finite-angles-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathBoundedFiniteAnglesCandidateBundleCli(
        ['--write', join(root, 'bundle'), '--json'],
        io,
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.join(''))).toMatchObject({ reproducibleExactNegativeBundle: true });
    expect(await runNegPathBoundedFiniteAnglesCandidateBundleCli(['--unknown'], io)).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-bounded-finite-angles-raw-');
    await writeFile(join(raw, 'path-bounded-finite-angles.json'), '{}\n', 'utf8');
    expect((await verifyNegPathBoundedFiniteAnglesCandidateBundleV1(raw)).reasonCodes).toEqual([
      'artifact-hash-mismatch',
    ]);
    const extra = await copyBundle('neg-path-bounded-finite-angles-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect((await verifyNegPathBoundedFiniteAnglesCandidateBundleV1(extra)).reasonCodes).toEqual([
      'artifact-set-mismatch',
    ]);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-link-');
    const sourcePath = join(directory, 'path-bounded-finite-angles.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathBoundedFiniteAnglesCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(writeNegPathBoundedFiniteAnglesCandidateBundleV1(directory)).rejects.toThrow(
      'unexpected entry',
    );
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-bounded-finite-angles',
      NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathBoundedFiniteAnglesCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-BOUNDED-FINITE-ANGLES-ALTERNATE-V1';
  ...
  it('rejects an accepted replacement at the same angle leaf because its oracle changed', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-signature-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-angles.json')));
    ...
    await coherentlyRewriteArtifact(directory, 'source-path-bounded-finite-angles', source);
    expect(
      (await verifyNegPathBoundedFiniteAnglesCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-delta-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-angles.json')));
    const benignControl = structuredClone(
      NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
    ) as unknown as JsonRecord;
    angleArray(benignControl)[2] = (3 * Math.PI) / 4;
    expect(parseArtifactContractV1(benignControl)).toMatchObject({ ok: true });
    angleArray(source)[2] = (3 * Math.PI) / 4;
    expect(
      boundedFiniteAnglesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
        source,
      ...
      '$.pathCandidate.segments[0].motion.anglesByCrease[0].angles[2]',
    ]);
    const replay = parseArtifactContractV1(source);
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-angles-bytes-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-angles.json')));
    angleArray(source)[0] = 'not-a-number';
    expect(
      boundedFiniteAnglesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_ANGLES_FOLD_CONTROL_SOURCE_V1,
        source,
