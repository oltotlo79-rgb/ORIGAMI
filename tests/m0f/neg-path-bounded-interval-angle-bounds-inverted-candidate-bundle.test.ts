...
import { runNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleCli } from '../../m0f/neg-path-bounded-interval-angle-bounds-inverted-candidate-bundle-cli.js';
import {
  ...
  NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_CASE_SPEC_V1,
  ...
  buildNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1,
  parseNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleLedgerV1,
...
} from '../../m0f/neg-path-bounded-interval-angle-bounds-inverted-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-bounded-interval-angle-bounds-inverted-candidate-bundle-ledger-v1.schema.json',
);
...
describe('NEG-PATH-MUTATION-BOUNDED-INTERVAL-ANGLE-BOUNDS-INVERTED candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification =
      await verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    ...
    expect(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one inverted bounded interval angle bound and the complete ordered parser oracle', () => {
    expect(
      boundedIntervalAngleBoundsInvertedJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_CASE_SPEC_V1.changedPaths);
    ...
    expect(result.error.map((issue) => issue.message)).toEqual([
      'lower bound must not exceed upper bound',
      'interval bound for e-hinge must contain both adjacent knot angles',
    ]);
  ...
  it('fixes absence of cardinality, finite-value, and motion-map diagnostics', () => {
    const result = parseArtifactContractV1(
      NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_CASE_SPEC_V1.sourceDocument,
    );
    if (result.ok) throw new TypeError('negative must reject');
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(join(BUNDLE_DIRECTORY, 'path-bounded-interval-angle-bounds-inverted.json')),
    ).toEqual(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleLedgerV1(committedLedger).ok,
    ).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted =
        parseNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleLedgerV1(changed).ok;
  ...
  it.each([
    ...
    changed[field] = true;
    expect(validate(changed)).toBe(false);
    expect(parseNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleLedgerV1(changed).ok).toBe(
      false,
    );
  });

  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1();
    expect(first.files).toEqual(
      buildNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1().files,
    );
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-BOUNDED-INTERVAL-ANGLE-BOUNDS-INVERTED',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(
      await runNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleCli(['--verify'], io),
    ).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(
      join(tmpdir(), 'neg-path-bounded-interval-angle-bounds-inverted-cli-'),
    );
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleCli(
    ...
    expect(
      await runNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleCli(['--unknown'], io),
    ).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-raw-');
    await writeFile(join(raw, 'path-bounded-interval-angle-bounds-inverted.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1(raw)).reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1(extra)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-link-');
    const sourcePath = join(directory, 'path-bounded-interval-angle-bounds-inverted.json');
    await rm(sourcePath);
    ...
    await expect(
      writeNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-bounded-interval-angle-bounds-inverted',
      NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1(directory))
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-BOUNDED-INTERVAL-ANGLE-BOUNDS-INVERTED-ALTERNATE-V1';
  ...
  it('rejects an accepted lower-bound replacement at the same leaf because its oracle changed', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bounds-inverted-signature-',
    );
    const source = record(
      await jsonFile(join(directory, 'path-bounded-interval-angle-bounds-inverted.json')),
    );
    array(intervalBounds(source)[0])[0] = -Math.PI / 4;
    expect(
      boundedIntervalAngleBoundsInvertedJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundsInvertedCandidateBundleV1(directory))
        .reasonCodes,
    ).toEqual(['source-unexpectedly-accepted', 'deterministic-regeneration-mismatch']);
  });

  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-delta-');
    const source = record(
      await jsonFile(join(directory, 'path-bounded-interval-angle-bounds-inverted.json')),
    );
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-interval-angle-bounds-inverted-bytes-');
    const source = record(
      await jsonFile(join(directory, 'path-bounded-interval-angle-bounds-inverted.json')),
    );
    array(intervalBounds(source)[0])[0] = Math.PI;
    expect(
      boundedIntervalAngleBoundsInvertedJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUNDS_INVERTED_FOLD_CONTROL_SOURCE_V1,
        source,
