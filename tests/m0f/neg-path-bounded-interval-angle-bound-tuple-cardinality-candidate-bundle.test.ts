...
import { runNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleCli } from '../../m0f/neg-path-bounded-interval-angle-bound-tuple-cardinality-candidate-bundle-cli.js';
import {
  NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CASE_SPEC_V1,
  ...
  buildNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1,
  parseNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleLedgerV1,
  verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1,
  writeNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1,
} from '../../m0f/neg-path-bounded-interval-angle-bound-tuple-cardinality-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-bounded-interval-angle-bound-tuple-cardinality-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-BOUNDED-INTERVAL-ANGLE-BOUND-TUPLE-CARDINALITY-MISMATCH candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification =
      await verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    ...
    expect(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one three-coordinate bounded interval angle bound and the sole parser oracle', () => {
    expect(
      boundedIntervalAngleBoundTupleCardinalityJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CASE_SPEC_V1.changedPaths);
    ...
    expect(result.error[0]?.message).toBe('must contain exactly 2 finite numbers');
    ...
    expect(array(boundedMotion(source).knotTimes)).toEqual([0, 0.5, 1]);
    expect(angleArray(source)).toEqual([0, Math.PI / 2, Math.PI]);
    expect(angleRows(source)).toHaveLength(1);
    expect(intervalBoundRows(source)).toHaveLength(1);
    expect(intervalBounds(source)).toEqual([
      [0, Math.PI / 2, Math.PI],
      [Math.PI / 2, Math.PI],
    ]);
  });
  ...
  it('fixes absence of all secondary path diagnostics', () => {
    ...
    if (result.ok) throw new TypeError('negative must reject');
    expect(result.error.map((issue) => issue.code)).toEqual(['invalid-tuple']);
  });
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(
        join(BUNDLE_DIRECTORY, 'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json'),
      ),
    ).toEqual(NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleLedgerV1(committedLedger)
        .ok,
    ).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted =
        parseNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleLedgerV1(changed).ok;
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1();
    expect(first.files).toEqual(
      buildNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1().files,
    );
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(
      await runNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleCli(['--verify'], io),
    ).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(
      join(tmpdir(), 'neg-path-bounded-interval-angle-bound-tuple-cardinality-cli-'),
    );
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleCli(
  ...
  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-bounded-interval-angle-bound-tuple-cardinality-raw-');
    await writeFile(
      join(raw, 'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json'),
      '{}\n',
      'utf8',
    );
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(raw))
        .reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-extra-',
    );
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(extra))
        .reasonCodes,
  ...
  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-link-',
    );
    const sourcePath = join(
      directory,
      'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json',
    );
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(directory))
        .reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(
      writeNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-writer-',
    );
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-accepted-',
    );
    await coherentlyRewriteArtifact(
      directory,
      'source-path-bounded-interval-angle-bound-tuple-cardinality',
      NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathBoundedIntervalAngleBoundTupleCardinalityCandidateBundleV1(directory))
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-control-',
    );
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-BOUNDED-INTERVAL-ANGLE-BOUND-TUPLE-ALTERNATE-V1';
  ...
  it('rejects an accepted two-coordinate replacement because its delta and oracle changed', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-signature-',
    );
    const source = record(
      await jsonFile(
        join(directory, 'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json'),
      ),
    );
    const firstBound = array(intervalBounds(source)[0]);
    firstBound.splice(2, 1);
    firstBound[1] = (3 * Math.PI) / 4;
    expect(
      boundedIntervalAngleBoundTupleCardinalityJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1,
        source,
      ),
    ).toEqual(['$.pathCandidate.segments[0].motion.intervalAngleBoundsByCrease[0].bounds[0][1]']);
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-delta-',
    );
    const source = record(
      await jsonFile(
        join(directory, 'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json'),
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle(
      'neg-path-bounded-interval-angle-bound-tuple-cardinality-bytes-',
    );
    const source = record(
      await jsonFile(
        join(directory, 'path-bounded-interval-angle-bound-tuple-cardinality-mismatch.json'),
    ...
    array(intervalBounds(source)[0])[2] = -Math.PI;
    expect(
      boundedIntervalAngleBoundTupleCardinalityJsonDifferencePaths(
        NEG_PATH_BOUNDED_INTERVAL_ANGLE_BOUND_TUPLE_CARDINALITY_FOLD_CONTROL_SOURCE_V1,
        source,
