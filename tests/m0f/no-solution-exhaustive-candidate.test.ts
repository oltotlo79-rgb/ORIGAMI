...
import { candidateBranchDigest, evaluateBoundedExhaustiveCandidate } from '../../m0f/no-solution-exhaustive-candidate';

const base = { schemaVersion: 1 as const, recordType: 'm0f-bounded-exhaustive-no-solution-candidate' as const,
  contractStatus: 'candidate' as const, domain: { name: 'single-hinge-v1', version: '1', cardinality: 2 },
...
describe('bounded exhaustive no-solution candidate', () => {
  ...
  it('fails closed on incomplete exploration', () => {
    const result = evaluateBoundedExhaustiveCandidate({ ...base, explored: { ...base.explored, count: 1 } });
