...
import { runNegPathBoundedFiniteKnotTimesCandidateBundleCli } from '../../m0f/neg-path-bounded-finite-knot-times-candidate-bundle-cli.js';
import {
  NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CASE_SPEC_V1,
  NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1,
  boundedFiniteKnotTimesJsonDifferencePaths,
  buildNegPathBoundedFiniteKnotTimesCandidateBundleV1,
  parseNegPathBoundedFiniteKnotTimesCandidateBundleLedgerV1,
  verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1,
  writeNegPathBoundedFiniteKnotTimesCandidateBundleV1,
} from '../../m0f/neg-path-bounded-finite-knot-times-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-bounded-finite-knot-times-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-BOUNDED-FINITE-KNOT-TIMES candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1>
  >;

  beforeAll(async () => {
    committedLedger = record(
      await jsonFile(
        join(BUNDLE_DIRECTORY, NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CANDIDATE_BUNDLE_LEDGER_FILENAME),
      ),
    );
    validate = new Ajv2020({ strict: true, allErrors: true }).compile(
      record(await jsonFile(SCHEMA_PATH)),
    );
    committedVerification = await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    expect(
      parseArtifactContractV1(NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1),
    ).toMatchObject({ ok: true });
    expect(NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one non-finite bounded knot time and the sole parser oracle', () => {
    expect(
      boundedFiniteKnotTimesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CASE_SPEC_V1.changedPaths);
    ...
    expect(array(boundedMotion(source).knotTimes)).toEqual([0, 0.5, 1, null]);
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(await jsonFile(join(BUNDLE_DIRECTORY, 'path-bounded-finite-knot-times.json'))).toEqual(
      NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_CASE_SPEC_V1.sourceDocument,
    );
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(parseNegPathBoundedFiniteKnotTimesCandidateBundleLedgerV1(committedLedger).ok).toBe(
      true,
    );
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted = parseNegPathBoundedFiniteKnotTimesCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathBoundedFiniteKnotTimesCandidateBundleLedgerV1(changed).ok).toBe(false);
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathBoundedFiniteKnotTimesCandidateBundleV1();
    expect(first.files).toEqual(buildNegPathBoundedFiniteKnotTimesCandidateBundleV1().files);
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-BOUNDED-FINITE-KNOT-TIMES',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathBoundedFiniteKnotTimesCandidateBundleCli(['--verify'], io)).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-bounded-finite-knot-times-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathBoundedFiniteKnotTimesCandidateBundleCli(
        ['--write', join(root, 'bundle'), '--json'],
        io,
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.join(''))).toMatchObject({ reproducibleExactNegativeBundle: true });
    expect(await runNegPathBoundedFiniteKnotTimesCandidateBundleCli(['--unknown'], io)).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-bounded-finite-knot-times-raw-');
    await writeFile(join(raw, 'path-bounded-finite-knot-times.json'), '{}\n', 'utf8');
    expect((await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(raw)).reasonCodes).toEqual([
      'artifact-hash-mismatch',
    ]);
    const extra = await copyBundle('neg-path-bounded-finite-knot-times-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect((await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(extra)).reasonCodes).toEqual(
      ['artifact-set-mismatch'],
    );
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-link-');
    const sourcePath = join(directory, 'path-bounded-finite-knot-times.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(writeNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory)).rejects.toThrow(
      'unexpected entry',
    );
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-bounded-finite-knot-times',
      NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-BOUNDED-FINITE-KNOT-TIMES-ALTERNATE-V1';
  ...
  it('rejects removal of the negative knot leaf even though it restores an accepted source', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-signature-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-knot-times.json')));
    array(boundedMotion(source).knotTimes).splice(3, 1);
    expect(
      boundedFiniteKnotTimesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    await coherentlyRewriteArtifact(directory, 'source-path-bounded-finite-knot-times', source);
    expect(
      (await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory)).reasonCodes,
    ).toEqual([
      'source-control-difference-mismatch',
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-delta-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-knot-times.json')));
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-knot-times-bytes-');
    const source = record(await jsonFile(join(directory, 'path-bounded-finite-knot-times.json')));
    array(boundedMotion(source).knotTimes)[3] = 'not-a-number';
    expect(
      boundedFiniteKnotTimesJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_KNOT_TIMES_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    await coherentlyRewriteArtifact(directory, 'source-path-bounded-finite-knot-times', source);
    const result = await verifyNegPathBoundedFiniteKnotTimesCandidateBundleV1(directory);
