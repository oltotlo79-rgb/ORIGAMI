...
import { runNegPathPolynomialEmptyCoefficientRowsCandidateBundleCli } from '../../m0f/neg-path-polynomial-empty-coefficient-rows-candidate-bundle-cli.js';
import {
  ...
  NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_CASE_SPEC_V1,
  NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1,
  polynomialEmptyCoefficientRowsJsonDifferencePaths,
  buildNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1,
  parseNegPathPolynomialEmptyCoefficientRowsCandidateBundleLedgerV1,
  verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1,
  writeNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1,
} from '../../m0f/neg-path-polynomial-empty-coefficient-rows-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-polynomial-empty-coefficient-rows-candidate-bundle-ledger-v1.schema.json',
);
...
describe('NEG-PATH-MUTATION-POLYNOMIAL-EMPTY-COEFFICIENT-ROWS candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification = await verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1();
  });
  ...
  it('keeps an owned accepted single-segment degree-one polynomial control fully anchored', () => {
    expect(
      parseArtifactContractV1(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1),
    ).toMatchObject({ ok: true });
    expect(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_POLYNOMIAL_COEFFICIENT_DEGREE_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_POLYNOMIAL_COEFFICIENT_DEGREE_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(
      withoutPathCandidate(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1),
    ).toEqual(
  ...
  it('fixes deletion of the sole coefficient row and sole complete parser oracle', () => {
    expect(
      polynomialEmptyCoefficientRowsJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1,
        NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_CASE_SPEC_V1.changedPaths);
    ...
    expect(result.error[0]?.message).toBe('requires at least one coefficient row');
    const source = structuredClone(
      NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_CASE_SPEC_V1.sourceDocument,
    ) as unknown as JsonRecord;
    expect(coefficientEntries(source, 0)).toHaveLength(1);
    expect(coefficientRows(source, 0)).toHaveLength(0);
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(join(BUNDLE_DIRECTORY, 'path-polynomial-empty-coefficient-rows.json')),
    ).toEqual(NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathPolynomialEmptyCoefficientRowsCandidateBundleLedgerV1(committedLedger).ok,
    ).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted =
        parseNegPathPolynomialEmptyCoefficientRowsCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathPolynomialEmptyCoefficientRowsCandidateBundleLedgerV1(changed).ok).toBe(
        false,
      );
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1();
    expect(first.files).toEqual(
      buildNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1().files,
    );
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-POLYNOMIAL-EMPTY-COEFFICIENT-ROWS',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathPolynomialEmptyCoefficientRowsCandidateBundleCli(['--verify'], io)).toBe(
      0,
    );
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-polynomial-empty-coefficient-rows-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathPolynomialEmptyCoefficientRowsCandidateBundleCli(
    ...
    expect(
      await runNegPathPolynomialEmptyCoefficientRowsCandidateBundleCli(['--unknown'], io),
    ).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-polynomial-empty-coefficient-rows-raw-');
    await writeFile(join(raw, 'path-polynomial-empty-coefficient-rows.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1(raw)).reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle('neg-path-polynomial-empty-coefficient-rows-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1(extra)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-link-');
    const sourcePath = join(directory, 'path-polynomial-empty-coefficient-rows.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(
      writeNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-polynomial-empty-coefficient-rows',
      NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathPolynomialEmptyCoefficientRowsCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-control-');
    const control = record(await jsonFile(join(directory, 'control-design.json')));
    control.contractId = 'CONTRACT-DESIGN-POLYNOMIAL-EMPTY-COEFFICIENT-ROWS-ALTERNATE-V1';
  ...
  it('rejects the same deleted-row delta when its complete parser oracle changes', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-signature-');
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-empty-coefficient-rows.json')),
    );
    coefficientRows(source, 0).push(null);
    expect(
      polynomialEmptyCoefficientRowsJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_EMPTY_COEFFICIENT_ROWS_DESIGN_CONTROL_SOURCE_V1,
        source,
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-delta-');
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-empty-coefficient-rows.json')),
    );
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-polynomial-empty-coefficient-rows-bytes-');
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-empty-coefficient-rows.json')),
    );
    expect(
