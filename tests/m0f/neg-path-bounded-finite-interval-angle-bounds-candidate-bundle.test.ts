...
import { runNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleCli } from '../../m0f/neg-path-bounded-finite-interval-angle-bounds-candidate-bundle-cli.js';
import {
  NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CASE_SPEC_V1,
  NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1,
  boundedFiniteIntervalAngleBoundsJsonDifferencePaths,
  buildNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1,
  parseNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleLedgerV1,
  verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1,
  writeNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1,
} from '../../m0f/neg-path-bounded-finite-interval-angle-bounds-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-bounded-finite-interval-angle-bounds-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-BOUNDED-FINITE-INTERVAL-ANGLE-BOUNDS candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification = await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    ...
    expect(NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one non-finite bounded interval angle bound and the sole parser oracle', () => {
    expect(
      boundedFiniteIntervalAngleBoundsJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CASE_SPEC_V1.changedPaths);
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(join(BUNDLE_DIRECTORY, 'path-bounded-finite-interval-angle-bounds.json')),
    ).toEqual(NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleLedgerV1(committedLedger).ok,
    ).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted =
        parseNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleLedgerV1(changed).ok).toBe(
        false,
      );
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1();
    expect(first.files).toEqual(
      buildNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1().files,
    );
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-BOUNDED-FINITE-INTERVAL-ANGLE-BOUNDS',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(
      await runNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleCli(['--verify'], io),
    ).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(
      join(tmpdir(), 'neg-path-bounded-finite-interval-angle-bounds-cli-'),
    );
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleCli(
    ...
    expect(
      await runNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleCli(['--unknown'], io),
    ).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-raw-');
    await writeFile(join(raw, 'path-bounded-finite-interval-angle-bounds.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(raw)).reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(extra)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-link-');
    const sourcePath = join(directory, 'path-bounded-finite-interval-angle-bounds.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(
      writeNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-bounded-finite-interval-angle-bounds',
      NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-BOUNDED-FINITE-INTERVAL-ANGLE-BOUNDS-ALTERNATE-V1';
  ...
  it('rejects an accepted replacement at the same interval-bound leaf because its oracle changed', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-signature-');
    const source = record(
      await jsonFile(join(directory, 'path-bounded-finite-interval-angle-bounds.json')),
    );
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-delta-');
    const source = record(
      await jsonFile(join(directory, 'path-bounded-finite-interval-angle-bounds.json')),
    );
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-bounded-finite-interval-angle-bounds-bytes-');
    const source = record(
      await jsonFile(join(directory, 'path-bounded-finite-interval-angle-bounds.json')),
    );
    array(intervalBounds(source)[0])[0] = 'not-a-number';
    expect(
      boundedFiniteIntervalAngleBoundsJsonDifferencePaths(
        NEG_PATH_BOUNDED_FINITE_INTERVAL_ANGLE_BOUNDS_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    const result = await verifyNegPathBoundedFiniteIntervalAngleBoundsCandidateBundleV1(directory);
