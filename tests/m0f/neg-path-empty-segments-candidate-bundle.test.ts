...
import { runNegPathEmptySegmentsCandidateBundleCli } from '../../m0f/neg-path-empty-segments-candidate-bundle-cli.js';
import {
  NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1,
  NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1,
  emptySegmentsJsonDifferencePaths,
  buildNegPathEmptySegmentsCandidateBundleV1,
  parseNegPathEmptySegmentsCandidateBundleLedgerV1,
  verifyNegPathEmptySegmentsCandidateBundleV1,
  writeNegPathEmptySegmentsCandidateBundleV1,
} from '../../m0f/neg-path-empty-segments-candidate-bundle.js';
import { parseFixtureManifest, sha256Prefixed } from '../../m0f/manifest.js';
import { stableStringify } from '../../m0f/stable-json.js';

const BUNDLE_DIRECTORY = resolve(NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY);
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-empty-segments-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathEmptySegmentsCandidateBundleV1(directory);
  return directory;
}
...
async function coherentlyRewriteArtifactText(
  ...
  const ledgerPath = join(directory, NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME);
...
describe('NEG-PATH-MUTATION-EMPTY-SEGMENTS candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathEmptySegmentsCandidateBundleV1>
  >;

  beforeAll(async () => {
    committedLedger = record(
      await jsonFile(
        join(BUNDLE_DIRECTORY, NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME),
      ),
    );
    validate = new Ajv2020({ strict: true, allErrors: true }).compile(
      record(await jsonFile(SCHEMA_PATH)),
    );
    committedVerification = await verifyNegPathEmptySegmentsCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    expect(parseArtifactContractV1(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1)).toMatchObject({
      ok: true,
    });
    expect(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes deletion of the sole path segment and the sole complete parser oracle', () => {
    expect(
      emptySegmentsJsonDifferencePaths(
        NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths);
    const result = parseArtifactContractV1(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument);
    ...
    expect(result.error[0]?.message).toBe('requires at least one segment');
    const source = structuredClone(
      NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument,
    ) as unknown as JsonRecord;
    expect(array(record(source.pathCandidate).segments)).toEqual([]);
  });

  it('fixes absence of all secondary path diagnostics', () => {
    const result = parseArtifactContractV1(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument);
    if (result.ok) throw new TypeError('negative must reject');
    expect(result.error.map((issue) => issue.code)).toEqual(['invalid-array']);
  });
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(await jsonFile(join(BUNDLE_DIRECTORY, 'path-empty-segments.json'))).toEqual(
      NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument,
    );
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(parseNegPathEmptySegmentsCandidateBundleLedgerV1(committedLedger).ok).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted = parseNegPathEmptySegmentsCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathEmptySegmentsCandidateBundleLedgerV1(changed).ok).toBe(false);
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathEmptySegmentsCandidateBundleV1();
    expect(first.files).toEqual(buildNegPathEmptySegmentsCandidateBundleV1().files);
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-EMPTY-SEGMENTS',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathEmptySegmentsCandidateBundleCli(['--verify'], io)).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-empty-segments-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathEmptySegmentsCandidateBundleCli(
        ['--write', join(root, 'bundle'), '--json'],
        io,
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.join(''))).toMatchObject({ reproducibleExactNegativeBundle: true });
    expect(await runNegPathEmptySegmentsCandidateBundleCli(['--unknown'], io)).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-empty-segments-raw-');
    await writeFile(join(raw, 'path-empty-segments.json'), '{}\n', 'utf8');
    expect((await verifyNegPathEmptySegmentsCandidateBundleV1(raw)).reasonCodes).toEqual([
      'artifact-hash-mismatch',
    ]);
    const extra = await copyBundle('neg-path-empty-segments-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect((await verifyNegPathEmptySegmentsCandidateBundleV1(extra)).reasonCodes).toEqual([
      'artifact-set-mismatch',
    ]);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-empty-segments-link-');
    const sourcePath = join(directory, 'path-empty-segments.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect((await verifyNegPathEmptySegmentsCandidateBundleV1(directory)).reasonCodes).toEqual([
      'artifact-set-mismatch',
    ]);
    await expect(writeNegPathEmptySegmentsCandidateBundleV1(directory)).rejects.toThrow(
      'unexpected entry',
    );
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-empty-segments-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-empty-segments-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-empty-segments',
      NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1,
    );
    expect((await verifyNegPathEmptySegmentsCandidateBundleV1(directory)).reasonCodes).toEqual([
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-empty-segments-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-EMPTY-SEGMENTS-CONTROL-ALTERNATE-V1';
  ...
  it('rejects the same segment-leaf delta when its complete parser oracle changes', async () => {
    const directory = await copyBundle('neg-path-empty-segments-signature-');
    const source = record(await jsonFile(join(directory, 'path-empty-segments.json')));
    record(source.pathCandidate).segments = [null];
    expect(
      emptySegmentsJsonDifferencePaths(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1, source),
    ).toEqual(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths);
    ...
    expect(replay.error.map((issue) => issue.message)).toEqual([
      'path segment must be an object',
      'segments must end at t=1',
    ]);
    await coherentlyRewriteArtifact(directory, 'source-path-empty-segments', source);
    expect((await verifyNegPathEmptySegmentsCandidateBundleV1(directory)).reasonCodes).toEqual([
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-empty-segments-delta-');
    const source = record(await jsonFile(join(directory, 'path-empty-segments.json')));
    const benignControl = structuredClone(
      NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1,
    ) as unknown as JsonRecord;
    benignControl.contractId = 'CONTRACT-FOLD-EMPTY-SEGMENTS-SOURCE-ALTERNATE-V1';
    expect(parseArtifactContractV1(benignControl)).toMatchObject({ ok: true });
    source.contractId = 'CONTRACT-FOLD-EMPTY-SEGMENTS-SOURCE-ALTERNATE-V1';
    expect(
      emptySegmentsJsonDifferencePaths(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1, source),
    ).toEqual(['$.contractId', '$.pathCandidate.segments[0]']);
    const replay = parseArtifactContractV1(source);
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-empty-segments-bytes-');
    const source = record(await jsonFile(join(directory, 'path-empty-segments.json')));
    expect(
      emptySegmentsJsonDifferencePaths(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1, source),
    ).toEqual(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths);
    ...
    await coherentlyRewriteArtifactText(directory, 'source-path-empty-segments', text);
    const result = await verifyNegPathEmptySegmentsCandidateBundleV1(directory);
