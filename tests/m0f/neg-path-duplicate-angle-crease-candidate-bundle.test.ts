...
import { runNegPathDuplicateAngleCreaseCandidateBundleCli } from '../../m0f/neg-path-duplicate-angle-crease-candidate-bundle-cli.js';
import {
  ...
  NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1,
  NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
  duplicateAngleCreaseJsonDifferencePaths,
  buildNegPathDuplicateAngleCreaseCandidateBundleV1,
  parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1,
  verifyNegPathDuplicateAngleCreaseCandidateBundleV1,
  writeNegPathDuplicateAngleCreaseCandidateBundleV1,
} from '../../m0f/neg-path-duplicate-angle-crease-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-duplicate-angle-crease-candidate-bundle-ledger-v1.schema.json',
);
...
describe('NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathDuplicateAngleCreaseCandidateBundleV1>
  >;

  beforeAll(async () => {
    committedLedger = record(
      await jsonFile(
        join(BUNDLE_DIRECTORY, NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME),
      ),
    );
    validate = new Ajv2020({ strict: true, allErrors: true }).compile(
      record(await jsonFile(SCHEMA_PATH)),
    );
    committedVerification = await verifyNegPathDuplicateAngleCreaseCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    expect(
      parseArtifactContractV1(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1),
    ).toMatchObject({ ok: true });
    expect(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one appended duplicate angle crease row and the sole complete parser oracle', () => {
    expect(
      duplicateAngleCreaseJsonDifferencePaths(
        NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.changedPaths);
    ...
    expect(angleRows(source)).toHaveLength(2);
    expect(angleRows(source)[1]).toEqual(angleRows(source)[0]);
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(await jsonFile(join(BUNDLE_DIRECTORY, 'path-duplicate-angle-crease.json'))).toEqual(
      NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourceDocument,
    );
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1(committedLedger).ok).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted = parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1(changed).ok).toBe(false);
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathDuplicateAngleCreaseCandidateBundleV1();
    expect(first.files).toEqual(buildNegPathDuplicateAngleCreaseCandidateBundleV1().files);
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathDuplicateAngleCreaseCandidateBundleCli(['--verify'], io)).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-duplicate-angle-crease-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathDuplicateAngleCreaseCandidateBundleCli(
        ['--write', join(root, 'bundle'), '--json'],
        io,
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.join(''))).toMatchObject({ reproducibleExactNegativeBundle: true });
    expect(await runNegPathDuplicateAngleCreaseCandidateBundleCli(['--unknown'], io)).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-duplicate-angle-crease-raw-');
    await writeFile(join(raw, 'path-duplicate-angle-crease.json'), '{}\n', 'utf8');
    expect((await verifyNegPathDuplicateAngleCreaseCandidateBundleV1(raw)).reasonCodes).toEqual([
      'artifact-hash-mismatch',
    ]);
    const extra = await copyBundle('neg-path-duplicate-angle-crease-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect((await verifyNegPathDuplicateAngleCreaseCandidateBundleV1(extra)).reasonCodes).toEqual([
      'artifact-set-mismatch',
    ]);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-link-');
    const sourcePath = join(directory, 'path-duplicate-angle-crease.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathDuplicateAngleCreaseCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(writeNegPathDuplicateAngleCreaseCandidateBundleV1(directory)).rejects.toThrow(
      'unexpected entry',
    );
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-duplicate-angle-crease',
      NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathDuplicateAngleCreaseCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-DUPLICATE-ANGLE-CREASE-ALTERNATE-V1';
  ...
  it('rejects the same appended-row delta when its complete parser oracle changes', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-signature-');
    const source = record(await jsonFile(join(directory, 'path-duplicate-angle-crease.json')));
    angleRows(source)[1] = null;
    expect(
      duplicateAngleCreaseJsonDifferencePaths(
        NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    expect(replay.error.map(({ path, code }) => ({ path, code }))).toEqual([
      {
        path: '$.pathCandidate.segments[0].motion.anglesByCrease[1]',
        code: 'invalid-object',
      },
    ]);
    expect(replay.error[0]?.message).toBe('crease angles must be an object');
    await coherentlyRewriteArtifact(directory, 'source-path-duplicate-angle-crease', source);
    expect(
      (await verifyNegPathDuplicateAngleCreaseCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-delta-');
    const source = record(await jsonFile(join(directory, 'path-duplicate-angle-crease.json')));
    const benignControl = structuredClone(
      NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
    ) as unknown as JsonRecord;
    array(intervalBounds(benignControl)[1])[1] = 2 * Math.PI;
    expect(parseArtifactContractV1(benignControl)).toMatchObject({ ok: true });
    array(intervalBounds(source)[1])[1] = 2 * Math.PI;
    expect(
      duplicateAngleCreaseJsonDifferencePaths(
        NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
        source,
      ...
      '$.pathCandidate.segments[0].motion.anglesByCrease[1]',
      '$.pathCandidate.segments[0].motion.intervalAngleBoundsByCrease[0].bounds[1][1]',
    ]);
    const replay = parseArtifactContractV1(source);
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-duplicate-angle-crease-bytes-');
    const source = record(await jsonFile(join(directory, 'path-duplicate-angle-crease.json')));
    const appendedAngles = angleArray(source, 1);
    appendedAngles[0] = Math.PI / 4;
    appendedAngles[1] = Math.PI / 2;
    appendedAngles[2] = Math.PI;
    expect(
      duplicateAngleCreaseJsonDifferencePaths(
        NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
        source,
