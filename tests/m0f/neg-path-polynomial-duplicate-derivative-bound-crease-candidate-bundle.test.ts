...
import { runNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleCli } from '../../m0f/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-cli.js';
import {
  NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1,
  ...
  buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1,
  parseNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1,
  verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1,
  writeNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1,
} from '../../m0f/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-ledger-v1.schema.json',
);
...
function derivativeBoundEntries(document: JsonRecord, segmentIndex: number): unknown[] {
  return array(segmentMotion(document, segmentIndex).derivativeBoundsByCrease);
}

function derivativeBoundRow(document: JsonRecord, segmentIndex: number, entryIndex = 0): unknown[] {
  return array(record(derivativeBoundEntries(document, segmentIndex)[entryIndex]).bounds);
}
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification =
      await verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1();
  });
  ...
  it('keeps an owned accepted single-segment degree-one polynomial control fully anchored', () => {
    ...
    expect(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_POLYNOMIAL_COEFFICIENT_DEGREE_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_POLYNOMIAL_COEFFICIENT_DEGREE_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(
    ...
    expect(derivativeBoundEntries(ownedControl, 0)).toHaveLength(1);
    expect(derivativeBoundRow(ownedControl, 0)).toEqual([0, Math.PI]);
  ...
  it('fixes one appended duplicate derivative-bound crease row and sole complete parser oracle', () => {
    expect(
      polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.changedPaths);
    ...
    expect(derivativeBoundEntries(source, 0)).toHaveLength(2);
    expect(derivativeBoundEntries(source, 0)[1]).toEqual(derivativeBoundEntries(source, 0)[0]);
  });
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(
        join(BUNDLE_DIRECTORY, 'path-polynomial-duplicate-derivative-bound-crease.json'),
      ),
    ).toEqual(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1(committedLedger)
        .ok,
    ).toBe(true);
  });
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(
        parseNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1(changed).ok,
      ).toBe(false);
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1();
    expect(first.files).toEqual(
      buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1().files,
    );
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) =>
          fixture.id === 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(
      await runNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleCli(['--verify'], io),
    ).toBe(0);
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(
      join(tmpdir(), 'neg-path-polynomial-duplicate-derivative-bound-crease-cli-'),
    );
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleCli(
    ...
    expect(
      await runNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleCli(['--unknown'], io),
    ).toBe(2);
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-polynomial-duplicate-derivative-bound-crease-raw-');
    await writeFile(
      join(raw, 'path-polynomial-duplicate-derivative-bound-crease.json'),
      '{}\n',
      'utf8',
    );
    expect(
      (await verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(raw))
        .reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle('neg-path-polynomial-duplicate-derivative-bound-crease-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(extra))
        .reasonCodes,
  ...
  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-link-',
    );
    const sourcePath = join(directory, 'path-polynomial-duplicate-derivative-bound-crease.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(directory))
        .reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(
      writeNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-writer-',
    );
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-accepted-',
    );
    await coherentlyRewriteArtifact(
      directory,
      'source-path-polynomial-duplicate-derivative-bound-crease',
      NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(directory))
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-control-',
    );
    const control = record(await jsonFile(join(directory, 'control-design.json')));
    control.contractId =
      'CONTRACT-DESIGN-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-ALTERNATE-V1';
  ...
  it('rejects the same appended-row delta when its complete parser oracle changes', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-signature-',
    );
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-duplicate-derivative-bound-crease.json')),
    );
    derivativeBoundEntries(source, 0)[1] = null;
    expect(
      polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
        source,
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-delta-',
    );
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-duplicate-derivative-bound-crease.json')),
    );
    coefficientRow(source, 0)[1] = Math.PI / 2;
    expect(
      polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
        source,
      ...
      '$.pathCandidate.segments[0].motion.derivativeBoundsByCrease[1]',
    ]);
    const replay = parseArtifactContractV1(source);
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle(
      'neg-path-polynomial-duplicate-derivative-bound-crease-bytes-',
    );
    const source = record(
      await jsonFile(join(directory, 'path-polynomial-duplicate-derivative-bound-crease.json')),
    );
    derivativeBoundRow(source, 0, 1)[1] = Math.PI / 2;
    expect(
      polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
        source,
