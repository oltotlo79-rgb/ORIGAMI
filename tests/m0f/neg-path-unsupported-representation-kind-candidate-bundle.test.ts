...
import { runNegPathUnsupportedRepresentationKindCandidateBundleCli } from '../../m0f/neg-path-unsupported-representation-kind-candidate-bundle-cli.js';
import {
  NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_LEDGER_FILENAME,
  NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1,
  NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
  unsupportedRepresentationKindJsonDifferencePaths,
  buildNegPathUnsupportedRepresentationKindCandidateBundleV1,
  parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1,
  verifyNegPathUnsupportedRepresentationKindCandidateBundleV1,
  writeNegPathUnsupportedRepresentationKindCandidateBundleV1,
} from '../../m0f/neg-path-unsupported-representation-kind-candidate-bundle.js';
...
const SCHEMA_PATH = resolve(
  'm0f/schemas/neg-path-unsupported-representation-kind-candidate-bundle-ledger-v1.schema.json',
);
...
async function copyBundle(prefix: string): Promise<string> {
  ...
  await writeNegPathUnsupportedRepresentationKindCandidateBundleV1(directory);
  return directory;
}
...
describe('NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND candidate bundle', () => {
  let committedLedger: JsonRecord;
  let validate: ReturnType<Ajv2020['compile']>;
  let committedVerification: Awaited<
    ReturnType<typeof verifyNegPathUnsupportedRepresentationKindCandidateBundleV1>
  >;
  ...
  beforeAll(async () => {
    ...
    committedVerification = await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1();
  });
  ...
  it('keeps the owned bounded-interpolation control fully anchored and freshly accepted', () => {
    expect(
      parseArtifactContractV1(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1),
    ).toMatchObject({ ok: true });
    expect(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1).not.toBe(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1).toEqual(
      NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1,
    );
  ...
  it('fixes one unsupported representation kind and the sole complete parser oracle', () => {
    expect(
      unsupportedRepresentationKindJsonDifferencePaths(
        NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
        NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourceDocument,
      ),
    ).toEqual(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.changedPaths);
    ...
    expect(result.error[0]?.message).toBe('unsupported path representation');
    const source = structuredClone(
      NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourceDocument,
    ) as unknown as JsonRecord;
    expect(record(source.pathCandidate).representationStatus).toBe('candidate');
    expect(boundedMotion(source).kind).toBe('spline');
  ...
  it('fixes absence of all secondary path diagnostics', () => {
    ...
    if (result.ok) throw new TypeError('negative must reject');
    expect(result.error.map((issue) => issue.code)).toEqual(['invalid-enum']);
  });
  ...
  it('matches the committed control and source exactly', async () => {
    ...
    expect(
      await jsonFile(join(BUNDLE_DIRECTORY, 'path-unsupported-representation-kind.json')),
    ).toEqual(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourceDocument);
  });
  ...
  it('strict-compiles the Draft 2020-12 schema and validates the runtime ledger', () => {
    ...
    expect(
      parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1(committedLedger).ok,
    ).toBe(true);
  });
  ...
  it('keeps strict Ajv and the runtime parser aligned across recursive rewrites', () => {
    ...
    for (const changed of mutations) {
      ...
      const runtimeAccepted =
        parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1(changed).ok;
  ...
  it('rejects unknown, missing, and changed case fields', () => {
    ...
    for (const changed of [extra, missing, caseChanged]) {
      ...
      expect(parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1(changed).ok).toBe(
        false,
      );
    }
  });
  ...
  it('builds the same closed four-file set twice', () => {
    const first = buildNegPathUnsupportedRepresentationKindCandidateBundleV1();
    expect(first.files).toEqual(buildNegPathUnsupportedRepresentationKindCandidateBundleV1().files);
  ...
  it('keeps the case outside the canonical manifest', async () => {
    ...
    expect(
      parsed.manifest?.fixtures.some(
        (fixture) => fixture.id === 'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND',
      ),
    ).toBe(false);
  });
  ...
  it('supports CLI verify/write and rejects invalid options', async () => {
    ...
    expect(await runNegPathUnsupportedRepresentationKindCandidateBundleCli(['--verify'], io)).toBe(
      0,
    );
    expect(stdout.join('')).toContain('REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE');
    const root = await mkdtemp(join(tmpdir(), 'neg-path-unsupported-representation-kind-cli-'));
    temporaryDirectories.push(root);
    stdout.length = 0;
    expect(
      await runNegPathUnsupportedRepresentationKindCandidateBundleCli(
        ['--write', join(root, 'bundle'), '--json'],
        io,
      ),
    ).toBe(0);
    expect(JSON.parse(stdout.join(''))).toMatchObject({ reproducibleExactNegativeBundle: true });
    expect(await runNegPathUnsupportedRepresentationKindCandidateBundleCli(['--unknown'], io)).toBe(
      2,
    );
  });

  it('rejects raw tampering and extra entries', async () => {
    const raw = await copyBundle('neg-path-unsupported-representation-kind-raw-');
    await writeFile(join(raw, 'path-unsupported-representation-kind.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(raw)).reasonCodes,
    ).toEqual(['artifact-hash-mismatch']);
    const extra = await copyBundle('neg-path-unsupported-representation-kind-extra-');
    await writeFile(join(extra, 'extra.json'), '{}\n', 'utf8');
    expect(
      (await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(extra)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
  });

  it('rejects links and unsafe writer cleanup', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-link-');
    const sourcePath = join(directory, 'path-unsupported-representation-kind.json');
    await rm(sourcePath);
    await symlink(BUNDLE_DIRECTORY, sourcePath, 'junction');
    expect(
      (await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(directory)).reasonCodes,
    ).toEqual(['artifact-set-mismatch']);
    await expect(
      writeNegPathUnsupportedRepresentationKindCandidateBundleV1(directory),
    ).rejects.toThrow('unexpected entry');
  });

  it('refuses cleanup of an unexpected regular file', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-writer-');
  ...
  it('rejects a coherently rehashed accepted-source rewrite', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-accepted-');
    await coherentlyRewriteArtifact(
      directory,
      'source-path-unsupported-representation-kind',
      NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
    );
    expect(
      (await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(directory)).reasonCodes,
  ...
  it('rejects a shifted but accepted saved-control anchor', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-control-');
    const control = record(await jsonFile(join(directory, 'control-fold.json')));
    control.contractId = 'CONTRACT-FOLD-UNSUPPORTED-REPRESENTATION-KIND-ALTERNATE-V1';
  ...
  it('rejects a missing motion field with a different delta and complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-signature-');
    const source = record(
      await jsonFile(join(directory, 'path-unsupported-representation-kind.json')),
    );
    const segment = record(array(record(source.pathCandidate).segments)[0]);
    delete segment.motion;
    expect(
      unsupportedRepresentationKindJsonDifferencePaths(
        NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
        source,
      ),
    ).toEqual(['$.pathCandidate.segments[0].motion']);
  ...
  it('rejects a wrong delta with the same complete parser oracle', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-delta-');
    const source = record(
      await jsonFile(join(directory, 'path-unsupported-representation-kind.json')),
    );
    ...
    expect(
      ...
      '$.pathCandidate.segments[0].motion.kind',
    ]);
    const replay = parseArtifactContractV1(source);
  ...
  it('rejects different bytes with the same delta and complete oracle', async () => {
    const directory = await copyBundle('neg-path-unsupported-representation-kind-bytes-');
    const source = record(
      await jsonFile(join(directory, 'path-unsupported-representation-kind.json')),
    );
    boundedMotion(source).kind = 'spline-v2';
    expect(
      unsupportedRepresentationKindJsonDifferencePaths(
        NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
        source,
    ...
    const result = await verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(directory);
