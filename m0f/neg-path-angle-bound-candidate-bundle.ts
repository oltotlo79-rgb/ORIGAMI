...
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-angle-bound-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-angle-bound-candidate-bundle-ledger' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-declared-bounded-interpolation-angle-containment-parser-replay-only' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-angle-bound-candidate-bundle' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_GENERATOR_VERSION = '1.0.0-candidate' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND' as const;
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_ANGLE_BOUND_CANONICAL_MANIFEST_PATH = 'tests/fixtures/manifest.json' as const;
export const NEG_PATH_ANGLE_BOUND_CASE_ID = 'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND' as const;

const SOURCE_REFERENCE_ID = 'oridesign-neg-path-angle-bound-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-angle-outside-bound' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathAngleBoundSourceDocumentV1 = NegLayerCycleSourceDocumentV1;
...
function boundedAngleFoldControl(): NegPathAngleBoundSourceDocumentV1 {
...
export const NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(boundedAngleFoldControl());

function pathAngleOutsideBoundSource(): NegPathAngleBoundSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1);
  const motion = requiredEntry(source.pathCandidate.segments, 0).motion;
  if (motion.kind !== 'bounded-interpolation') {
    throw new TypeError('fixed segment must use bounded interpolation');
  }
  const angles = requiredEntry(motion.anglesByCrease, 0).angles;
  requiredEntry(angles, 1);
  angles[1] = (3 * Math.PI) / 4;
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_ANGLE_BOUND_CASE_ID,
  ...
  sourceDocument: pathAngleOutsideBoundSource(),
  expectedIssues: [
    {
...
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND exact-negative candidate vector
...
export type NegPathAngleBoundArtifactProvenanceV1 = Readonly<{
...
export type NegPathAngleBoundArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathAngleBoundArtifactProvenanceV1;
}>;

type ArtifactSpecV1 = Omit<NegPathAngleBoundArtifactLedgerEntryV1, 'sha256'>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathAngleBoundArtifactProvenanceV1 {
  return {
...
export const NEG_PATH_ANGLE_BOUND_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathAngleBoundCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_ANGLE_BOUND_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0].motion.anglesByCrease[0].angles[1]'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND project-authored bounded-interpolation angle-containment parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathAngleBoundCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_ANGLE_BOUND_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathAngleBoundCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND-V1';
  scope: typeof NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCOPE;
  declaredBoundedInterpolationAngleContainmentParserOnly: true;
  canonicalManifestPath: typeof NEG_PATH_ANGLE_BOUND_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathAngleBoundCaseLedgerRowV1[];
  artifacts: readonly NegPathAngleBoundArtifactLedgerEntryV1[];
}>;

export type NegPathAngleBoundCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathAngleBoundCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathAngleBoundCandidateBundleLedgerV1 }>
  | Readonly<{ ok: false; error: readonly NegPathAngleBoundCandidateLedgerIssueV1[] }>;
...
function addLedgerIssue(
  issues: NegPathAngleBoundCandidateLedgerIssueV1[],
  path: string,
  code: NegPathAngleBoundCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathAngleBoundCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathAngleBoundCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathAngleBoundCandidateLedgerIssueV1[] = [];
  exactKeys(value, ROOT_KEYS, '$', issues);
  const literals = [
    ['schemaVersion', 1],
    ['schemaId', NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCHEMA_ID],
    ['recordType', NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_RECORD_TYPE],
    ['contractStatus', 'candidate'],
    ['scientificClaim', false],
    ['vectorSetId', 'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND-V1'],
    ['scope', NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCOPE],
    ['declaredBoundedInterpolationAngleContainmentParserOnly', true],
    ['canonicalManifestPath', NEG_PATH_ANGLE_BOUND_CANONICAL_MANIFEST_PATH],
    ['canonicalManifestRegistration', 'not-registered'],
    ['canonicalPromotionClaimed', false],
    ['canonicalPathMutationFamilyComplete', false],
    ['physicalAngleBoundsEstablished', false],
  ...
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [index, expected] of NEG_PATH_ANGLE_BOUND_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathAngleBoundCandidateBundleLedgerV1,
  });
}
...
export type NegPathAngleBoundCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathAngleBoundCandidateBuildV1 = Readonly<{
  ledger: NegPathAngleBoundCandidateBundleLedgerV1;
  files: readonly NegPathAngleBoundCandidateBuiltFileV1[];
}>;

/** Builds only after the accepted control, exact one-path delta, and parser oracle replay. */
export function buildNegPathAngleBoundCandidateBundleV1(): NegPathAngleBoundCandidateBuildV1 {
  if (!parseArtifactContractV1(NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1).ok) {
    throw new TypeError(
      'accepted NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND fold control no longer parses',
    );
  }
  ...
  if (!sameStableJson(differences, NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.changedPaths)) {
    throw new TypeError('NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND control delta changed');
  }
  const replay = parseArtifactContractV1(NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.sourceDocument);
  if (
    replay.ok ||
    !sameStableJson(issueSignature(replay.error), NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.expectedIssues)
  ) {
    throw new TypeError('NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_ANGLE_BOUND_CANDIDATE_README_V1],
    [CONTROL_ARTIFACT_ID, jsonLine(NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1)],
    [SOURCE_ARTIFACT_ID, jsonLine(NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.sourceDocument)],
  ]);
  const artifacts = NEG_PATH_ANGLE_BOUND_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing path-angle-outside-bound artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathAngleBoundCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathAngleBoundCandidateBuiltFileV1[] = artifacts.map((artifact) => {
    const text = textByArtifactId.get(artifact.artifactId);
    if (text === undefined)
      throw new TypeError(`missing path-angle-outside-bound artifact ${artifact.artifactId}`);
    return { artifactId: artifact.artifactId, path: artifact.path, text };
  });
  files.push({
    artifactId: null,
    path: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_LEDGER_FILENAME,
...
export async function writeNegPathAngleBoundCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathAngleBoundCandidateBuildV1> {
  const built = buildNegPathAngleBoundCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path-angle-outside-bound candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_ANGLE_BOUND_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathAngleBoundCandidateReasonCodeV1 =
  (typeof NEG_PATH_ANGLE_BOUND_CANDIDATE_REASON_CODES)[number];

export type NegPathAngleBoundCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-angle-bound-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND-V1';
  scope: typeof NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathAngleBoundCandidateReasonCodeV1[];
  checks: Readonly<{
...
interface MutableVerificationState {
  reasons: Set<NegPathAngleBoundCandidateReasonCodeV1>;
  ledgerPresentAndValid: boolean;
  artifactSetExact: boolean;
  allArtifactHashesMatch: boolean;
  allArtifactProvenanceFixed: boolean;
...
): NegPathAngleBoundCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_ANGLE_BOUND_CANDIDATE_REASON_CODES.filter((code) =>
    state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType: 'm0f-neg-path-angle-bound-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-ANGLE-OUTSIDE-BOUND-V1' as const,
    scope: NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_SCOPE,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(resolve(NEG_PATH_ANGLE_BOUND_CANONICAL_MANIFEST_PATH), 'utf8'),
      ) as unknown,
    );
    if (parsed.manifest === undefined) return undefined;
    return !parsed.manifest.fixtures.some((fixture) => fixture.id === NEG_PATH_ANGLE_BOUND_CASE_ID);
  } catch {
    return undefined;
  }
}

/** Replays the saved control and source only through the declared bounded-angle parser boundary. */
export async function verifyNegPathAngleBoundCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathAngleBoundCandidateVerificationResultV1> {
  ...
  try {
    ...
    try {
      ledgerText = await readFile(
        resolve(absoluteDirectory, NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_LEDGER_FILENAME),
        'utf8',
      );
    ...
    const parsedLedger = parseNegPathAngleBoundCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_ANGLE_BOUND_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        ...
        sameStableJson(
          ...
          NEG_PATH_ANGLE_BOUND_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathAngleBoundCandidateBundleV1();
