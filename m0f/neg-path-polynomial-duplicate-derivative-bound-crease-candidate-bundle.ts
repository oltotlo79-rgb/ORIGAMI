...
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-ledger' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-piecewise-polynomial-unique-derivative-bound-crease-rows-parser-replay-only' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_ID =
  'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE' as const;

const SOURCE_REFERENCE_ID =
  'oridesign-neg-path-polynomial-duplicate-derivative-bound-crease-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-design' as const;
const SOURCE_ARTIFACT_ID = 'source-path-polynomial-duplicate-derivative-bound-crease' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathPolynomialDuplicateDerivativeBoundCreaseSourceDocumentV1 =
  NegPathPolynomialCoefficientDegreeSourceDocumentV1;

type PolynomialDuplicateDerivativeBoundCreasePathSegmentV1 =
  NegPathPolynomialDuplicateDerivativeBoundCreaseSourceDocumentV1['pathCandidate']['segments'][number];

function isFixedDegreeOnePolynomialSegment(
  segment: PolynomialDuplicateDerivativeBoundCreasePathSegmentV1 | undefined,
  t0: number,
  t1: number,
): segment is PolynomialDuplicateDerivativeBoundCreasePathSegmentV1 {
  if (
    segment?.t0 !== t0 ||
    segment.t1 !== t1 ||
...
function polynomialDuplicateDerivativeBoundCreaseControl(): NegPathPolynomialDuplicateDerivativeBoundCreaseSourceDocumentV1 {
...
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1 =
  deepFreezeOwned(polynomialDuplicateDerivativeBoundCreaseControl());

function polynomialDuplicateDerivativeBoundCreaseSource(): NegPathPolynomialDuplicateDerivativeBoundCreaseSourceDocumentV1 {
  const source = structuredClone(
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
  );
  ...
  const originalDerivativeBoundEntry = firstSegment.motion.derivativeBoundsByCrease[0];
  if (originalDerivativeBoundEntry === undefined) {
    throw new TypeError('fixed polynomial derivative-bound row is missing');
  }
  firstSegment.motion.derivativeBoundsByCrease.push(structuredClone(originalDerivativeBoundEntry));
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'append-duplicate-polynomial-derivative-bound-crease-row',
  changedPaths: ['$.pathCandidate.segments[0].motion.derivativeBoundsByCrease[1]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-polynomial-duplicate-derivative-bound-crease.json',
  sourceDocument: polynomialDuplicateDerivativeBoundCreaseSource(),
  expectedIssues: [
    {
      path: '$.pathCandidate.segments[0].motion.derivativeBoundsByCrease[1].edgeId',
      code: 'duplicate-reference',
    },
  ],
} as const);

export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE exact-negative candidate vector

This closed directory contains one saved project-authored single-segment degree-one piecewise-polynomial design control and one exact-negative mutation for the current \`parseArtifactContractV1\` unique derivative-bound crease-row check. The accepted control is an owned clone with one valid \`e-hinge\` coefficient row \`[[0,pi]]\` and derivative bounds \`[0,pi]\`. The mutation appends only a cloned derivative-bound row at index 1. It produces exactly the sole \`duplicate-reference\` issue at the duplicate row's \`edgeId\`; the coefficient and derivative crease-reference sets remain equal, so there is no motion-map secondary issue.

The bundle is intentionally outside \`tests/fixtures/manifest.json\`; \`NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE\` is not canonically registered or promoted. Exact-negative means only that the current artifact-contract parser rejects these saved bytes with the complete ordered issue signature fixed in the ledger.

This replay establishes only rejection of the saved duplicate derivative-bound crease-row reference. It does not establish coefficient-row uniqueness, coefficient/degree cardinality, path-time coverage or physical continuity, select a representation, freeze a polynomial basis, establish coefficient ordering or semantics, associate coefficients with time intervals, establish derivative semantics or validation, infer polynomial endpoints, prove motion-map or crease-map completeness, rigidity, face isometry, hinge geometry, contact analysis, CCD, collision detection or freedom, foldability, certificate-hash verification, cryptographic authenticity, canonical path-mutation-family completeness, SupportProfile, ToleranceProfile, scientific verification, or M0F GO. Local SHA-256 rows detect saved-byte drift only and are not signatures.

Verify with \`npx tsx m0f/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-cli.ts --verify\`. Regenerate with \`npx tsx m0f/neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-cli.ts --write\`.
`;

export type NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactProvenanceV1 = Readonly<{
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<
  NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactLedgerEntryV1,
  'sha256'
>;

export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_ARTIFACT_SPECS_V1 =
  deepFreezeOwned([
    {
    ...
    },
    {
      ...
      path: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourcePath,
      mediaType: 'application/json',
      licenseSpdx: 'MIT',
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0].motion.derivativeBoundsByCrease[1]'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion:
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE project-authored piecewise-polynomial unique derivative-bound crease-row parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathPolynomialDuplicateDerivativeBoundCreaseCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues:
        NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-V1';
  scope: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE;
  declaredPiecewisePolynomialUniqueDerivativeBoundCreaseRowsParserOnly: true;
  ...
  canonicalManifestPath: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathPolynomialDuplicateDerivativeBoundCreaseCaseLedgerRowV1[];
  artifacts: readonly NegPathPolynomialDuplicateDerivativeBoundCreaseArtifactLedgerEntryV1[];
}>;

export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerParseResultV1 =
  | Readonly<{
      ok: true;
      value: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1;
    }>
  | Readonly<{
      ok: false;
      error: readonly NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerIssueV1[];
    }>;
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID],
  [
    'recordType',
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE,
  ],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-V1'],
  ['scope', NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE],
  ['declaredPiecewisePolynomialUniqueDerivativeBoundCreaseRowsParserOnly', true],
  ['declaredPiecewisePolynomialUniqueCoefficientCreaseRowsParserOnly', false],
  ['declaredPiecewisePolynomialCoefficientDegreeCardinalityParserOnly', false],
  ['pathTimeCoverageEstablished', false],
  [
    'canonicalManifestPath',
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANONICAL_MANIFEST_PATH,
  ],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
...
function addLedgerIssue(
  issues: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerIssueV1[],
  path: string,
  code: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [
      index,
      expected,
    ] of NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value:
      value as unknown as NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1,
  });
}
...
export function polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
            left[index],
            right[index],
            childPath,
          ),
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuildV1 = Readonly<{
  ledger: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1;
  files: readonly NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuiltFileV1[];
}>;

export function buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(): NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuildV1 {
  if (
    !parseArtifactContractV1(
      NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
    ).ok
  ) {
    throw new TypeError('accepted polynomial duplicate-derivative-bound control no longer parses');
  }
  const differences = polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1,
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourceDocument,
  );
  if (
    !sameStableJson(
      differences,
      NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.changedPaths,
    )
  ) {
    throw new TypeError('polynomial duplicate-derivative-bound control delta changed');
  }
  ...
  if (
    ...
    throw new TypeError('polynomial duplicate-derivative-bound parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_README_V1],
    [
      CONTROL_ARTIFACT_ID,
      jsonLine(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_DESIGN_CONTROL_SOURCE_V1),
    ],
    [
      SOURCE_ARTIFACT_ID,
      jsonLine(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.sourceDocument),
    ],
  ]);
  const artifacts = NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_ARTIFACT_SPECS_V1.map(
    (spec) => {
      const text = textByArtifactId.get(spec.artifactId);
      if (text === undefined)
        throw new TypeError(
          `missing polynomial duplicate-derivative-bound artifact ${spec.artifactId}`,
        );
      return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
    },
  );
  const ledger: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
    scientificClaim: false,
    vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-V1',
    scope: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE,
  ...
  const files: NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuiltFileV1[] =
    artifacts.map((artifact) => {
      const text = textByArtifactId.get(artifact.artifactId);
      if (text === undefined)
        throw new TypeError(
          `missing polynomial duplicate-derivative-bound artifact ${artifact.artifactId}`,
        );
      return { artifactId: artifact.artifactId, path: artifact.path, text };
    });
  files.push({
    artifactId: null,
    path: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBuildV1> {
  const built = buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path polynomial duplicate-derivative-bound candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateReasonCodeV1 =
  (typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_REASON_CODES)[number];
...
export type NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateVerificationResultV1 =
  Readonly<{
    schemaVersion: 1;
    recordType: 'm0f-neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-verification-result';
    contractStatus: 'candidate';
    scientificClaim: false;
    vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-V1';
    scope: typeof NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE;
    ...
    reasonCodes: readonly NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateReasonCodeV1[];
    checks: VerificationChecks;
  }>;

interface MutableVerificationState {
  reasons: Set<NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateReasonCodeV1>;
...
): NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateVerificationResultV1 {
  const reasonCodes =
    NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_REASON_CODES.filter((code) =>
      state.reasons.has(code),
    );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType:
      'm0f-neg-path-polynomial-duplicate-derivative-bound-crease-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-DUPLICATE-DERIVATIVE-BOUND-CREASE-V1' as const,
    scope: NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE,
    declaredPiecewisePolynomialUniqueDerivativeBoundCreaseRowsParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(
          resolve(NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANONICAL_MANIFEST_PATH),
          'utf8',
        ),
      ) as unknown,
    );
    ...
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathPolynomialDuplicateDerivativeBoundCreaseCandidateVerificationResultV1> {
  ...
  try {
    ...
    try {
      ledgerText = await readFile(
        resolve(
          absoluteDirectory,
          NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
        ),
        'utf8',
      );
    ...
    const parsedLedger =
      parseNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          polynomialDuplicateDerivativeBoundCreaseJsonDifferencePaths(control, source),
          NEG_PATH_POLYNOMIAL_DUPLICATE_DERIVATIVE_BOUND_CREASE_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathPolynomialDuplicateDerivativeBoundCreaseCandidateBundleV1();
