...
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-polynomial-motion-map-mismatch-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-polynomial-motion-map-mismatch-candidate-bundle-ledger' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-piecewise-polynomial-motion-map-pairing-parser-replay-only' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-polynomial-motion-map-mismatch-candidate-bundle' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_ID =
  'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH' as const;

const SOURCE_REFERENCE_ID =
  'oridesign-neg-path-polynomial-motion-map-mismatch-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-design' as const;
const SOURCE_ARTIFACT_ID = 'source-path-polynomial-motion-map-mismatch' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathPolynomialMotionMapMismatchSourceDocumentV1 =
  NegPathEndpointMapMismatchSourceDocumentV1;

function polynomialMotionMapDesignControl(): NegPathPolynomialMotionMapMismatchSourceDocumentV1 {
  const control = structuredClone(NEG_PATH_ENDPOINT_MAP_MISMATCH_DESIGN_CONTROL_SOURCE_V1);
  ...
  const replay = parseArtifactContractV1(control);
  if (!replay.ok) {
    throw new TypeError('fixed two-hinge polynomial design control must be accepted');
  }
  return control;
}

export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_DESIGN_CONTROL_SOURCE_V1 = deepFreezeOwned(
  polynomialMotionMapDesignControl(),
);

function polynomialMotionMapMismatchSource(): NegPathPolynomialMotionMapMismatchSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_DESIGN_CONTROL_SOURCE_V1);
  ...
  const coefficientRows = firstSegment.motion.coefficientsByCrease;
  const derivativeRows: (typeof firstSegment.motion.derivativeBoundsByCrease)[number][] =
    firstSegment.motion.derivativeBoundsByCrease;
  if (
    coefficientRows.length !== 2 ||
    coefficientRows[0]?.edgeId !== 'e-hinge' ||
    coefficientRows[1]?.edgeId !== 'e-hinge-far' ||
    coefficientRows.some((entry) => {
      const row = entry.coefficients[0];
      return row?.length !== 2 || row[0] !== 0 || row[1] !== Math.PI;
    })
  ) {
    throw new TypeError('fixed coefficient rows must remain two-hinge [0, pi] rows');
  }
  if (
    derivativeRows.length !== 2 ||
    derivativeRows[0]?.edgeId !== 'e-hinge' ||
    derivativeRows[1]?.edgeId !== 'e-hinge-far' ||
    derivativeRows.some((entry) => entry.bounds[0] !== 0 || entry.bounds[1] !== Math.PI)
  ) {
    throw new TypeError('fixed derivative bounds must remain two-hinge [0, pi] rows');
  }
  const removed = derivativeRows.splice(1, 1);
  if (removed[0]?.edgeId !== 'e-hinge-far' || Array.from(derivativeRows).length !== 1) {
    throw new TypeError('fixed mutation must remove only the far-hinge derivative row');
  }
  return source;
}
...
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'remove-far-hinge-polynomial-derivative-bound-row',
  changedPaths: ['$.pathCandidate.segments[0].motion.derivativeBoundsByCrease[1]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-polynomial-motion-map-mismatch.json',
  sourceDocument: polynomialMotionMapMismatchSource(),
  expectedIssues: [
    {
...
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH exact-negative candidate vector
...
export type NegPathPolynomialMotionMapMismatchArtifactProvenanceV1 = Readonly<{
...
export type NegPathPolynomialMotionMapMismatchArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathPolynomialMotionMapMismatchArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathPolynomialMotionMapMismatchArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathPolynomialMotionMapMismatchArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathPolynomialMotionMapMismatchCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.mutationKind;
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH project-authored piecewise-polynomial motion-map pairing parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathPolynomialMotionMapMismatchCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH-V1';
  scope: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCOPE;
  ...
  canonicalManifestPath: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathPolynomialMotionMapMismatchCaseLedgerRowV1[];
  artifacts: readonly NegPathPolynomialMotionMapMismatchArtifactLedgerEntryV1[];
}>;

export type NegPathPolynomialMotionMapMismatchCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathPolynomialMotionMapMismatchCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1 }>
  | Readonly<{
      ok: false;
      error: readonly NegPathPolynomialMotionMapMismatchCandidateLedgerIssueV1[];
    }>;
...
const ROOT_KEYS = [
  ...
  'vectorSetId',
  'scope',
  'declaredPiecewisePolynomialMotionMapPairingParserOnly',
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH-V1'],
  ['scope', NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCOPE],
  ['declaredPiecewisePolynomialMotionMapPairingParserOnly', true],
  ['declaredPiecewisePolynomialCoefficientDegreeCardinalityParserOnly', false],
  ['canonicalManifestPath', NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['representationSelectionEstablished', false],
...
function addLedgerIssue(
  issues: NegPathPolynomialMotionMapMismatchCandidateLedgerIssueV1[],
  path: string,
  code: NegPathPolynomialMotionMapMismatchCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathPolynomialMotionMapMismatchCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathPolynomialMotionMapMismatchCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.cases) || value.cases.length !== 1) {
    ...
    addLedgerIssue(
      ...
      '$.cases[0]',
      'invalid-case',
      'case identity, control link, derivative-row deletion delta, path, or complete issue oracle differs',
    );
  }
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [
      index,
      expected,
    ] of NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1,
  });
}
...
export function polynomialMotionMapMismatchJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...polynomialMotionMapMismatchJsonDifferencePaths(left[index], right[index], childPath),
        );
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      if (!Object.hasOwn(left, key) || !Object.hasOwn(right, key)) differences.push(childPath);
        ...
        differences.push(
          ...polynomialMotionMapMismatchJsonDifferencePaths(left[key], right[key], childPath),
        );
    }
    return differences;
  }
...
export type NegPathPolynomialMotionMapMismatchCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathPolynomialMotionMapMismatchCandidateBuildV1 = Readonly<{
  ledger: NegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1;
  files: readonly NegPathPolynomialMotionMapMismatchCandidateBuiltFileV1[];
}>;

export function buildNegPathPolynomialMotionMapMismatchCandidateBundleV1(): NegPathPolynomialMotionMapMismatchCandidateBuildV1 {
  if (
    !parseArtifactContractV1(NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_DESIGN_CONTROL_SOURCE_V1).ok
  ) {
    throw new TypeError('accepted polynomial motion-map control no longer parses');
  }
  const differences = polynomialMotionMapMismatchJsonDifferencePaths(
  ...
  if (
    !sameStableJson(differences, NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.changedPaths)
  ) {
    throw new TypeError('polynomial motion-map row deletion delta changed');
  }
  ...
  if (
    ...
    throw new TypeError('polynomial motion-map parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_README_V1],
    [
      CONTROL_ARTIFACT_ID,
      jsonLine(NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_DESIGN_CONTROL_SOURCE_V1),
    ],
    [
      SOURCE_ARTIFACT_ID,
      jsonLine(NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.sourceDocument),
    ],
  ]);
  const artifacts = NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing polynomial motion-map artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathPolynomialMotionMapMismatchCandidateBuiltFileV1[] = artifacts.map(
    (artifact) => {
      const text = textByArtifactId.get(artifact.artifactId);
      if (text === undefined)
        throw new TypeError(`missing polynomial motion-map artifact ${artifact.artifactId}`);
      return { artifactId: artifact.artifactId, path: artifact.path, text };
    },
  );
  files.push({
    artifactId: null,
    path: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathPolynomialMotionMapMismatchCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathPolynomialMotionMapMismatchCandidateBuildV1> {
  const built = buildNegPathPolynomialMotionMapMismatchCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path polynomial motion-map candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathPolynomialMotionMapMismatchCandidateReasonCodeV1 =
  (typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_REASON_CODES)[number];
...
export type NegPathPolynomialMotionMapMismatchCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-polynomial-motion-map-mismatch-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH-V1';
  scope: typeof NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathPolynomialMotionMapMismatchCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathPolynomialMotionMapMismatchCandidateReasonCodeV1>;
...
): NegPathPolynomialMotionMapMismatchCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_REASON_CODES.filter(
    (code) => state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType:
      'm0f-neg-path-polynomial-motion-map-mismatch-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-POLYNOMIAL-MOTION-MAP-MISMATCH-V1' as const,
    scope: NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_SCOPE,
    declaredPiecewisePolynomialMotionMapPairingParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(
          resolve(NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANONICAL_MANIFEST_PATH),
          'utf8',
        ),
      ) as unknown,
    );
    ...
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathPolynomialMotionMapMismatchCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathPolynomialMotionMapMismatchCandidateVerificationResultV1> {
  ...
  try {
    ...
    const parsedLedger =
      parseNegPathPolynomialMotionMapMismatchCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          polynomialMotionMapMismatchJsonDifferencePaths(control, source),
          NEG_PATH_POLYNOMIAL_MOTION_MAP_MISMATCH_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathPolynomialMotionMapMismatchCandidateBundleV1();
