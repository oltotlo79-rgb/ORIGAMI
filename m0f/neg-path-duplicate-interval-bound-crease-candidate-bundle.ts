...
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-duplicate-interval-bound-crease-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-duplicate-interval-bound-crease-candidate-bundle-ledger' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-bounded-interpolation-unique-interval-bound-crease-rows-parser-replay-only' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-duplicate-interval-bound-crease-candidate-bundle' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_ID =
  'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE' as const;

const SOURCE_REFERENCE_ID =
  'oridesign-neg-path-duplicate-interval-bound-crease-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-duplicate-interval-bound-crease' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathDuplicateIntervalBoundCreaseSourceDocumentV1 = NegPathAngleBoundSourceDocumentV1;

function ownedAcceptedControl(): NegPathDuplicateIntervalBoundCreaseSourceDocumentV1 {
...
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(ownedAcceptedControl());

function duplicateIntervalBoundCreaseSource(): NegPathDuplicateIntervalBoundCreaseSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_FOLD_CONTROL_SOURCE_V1);
  ...
  if (
    firstSegment.motion.intervalAngleBoundsByCrease.length !== 1 ||
    firstBounds?.edgeId !== 'e-hinge' ||
    firstBounds.bounds.length !== 2 ||
    firstBounds.bounds[0]?.[0] !== 0 ||
    firstBounds.bounds[0][1] !== Math.PI / 2 ||
    firstBounds.bounds[1]?.[0] !== Math.PI / 2 ||
    firstBounds.bounds[1][1] !== Math.PI
  ) {
    throw new TypeError('fixed valid interval-bound row is missing');
  }
  firstSegment.motion.intervalAngleBoundsByCrease.push(structuredClone(firstBounds));
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'append-duplicate-bounded-interpolation-interval-bound-crease-row',
  changedPaths: ['$.pathCandidate.segments[0].motion.intervalAngleBoundsByCrease[1]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-duplicate-interval-bound-crease.json',
  sourceDocument: duplicateIntervalBoundCreaseSource(),
  expectedIssues: [
    {
      path: '$.pathCandidate.segments[0].motion.intervalAngleBoundsByCrease[1].edgeId',
      code: 'duplicate-reference',
    },
  ],
} as const);

export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE exact-negative candidate vector
...
export type NegPathDuplicateIntervalBoundCreaseArtifactProvenanceV1 = Readonly<{
...
export type NegPathDuplicateIntervalBoundCreaseArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathDuplicateIntervalBoundCreaseArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathDuplicateIntervalBoundCreaseArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathDuplicateIntervalBoundCreaseArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathDuplicateIntervalBoundCreaseCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.mutationKind;
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE project-authored bounded-interpolation unique interval-bound crease-row parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathDuplicateIntervalBoundCreaseCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE-V1';
  scope: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE;
  ...
  canonicalManifestPath: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathDuplicateIntervalBoundCreaseCaseLedgerRowV1[];
  artifacts: readonly NegPathDuplicateIntervalBoundCreaseArtifactLedgerEntryV1[];
}>;

export type NegPathDuplicateIntervalBoundCreaseCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathDuplicateIntervalBoundCreaseCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1 }>
  | Readonly<{
      ok: false;
      error: readonly NegPathDuplicateIntervalBoundCreaseCandidateLedgerIssueV1[];
    }>;
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE-V1'],
  ['scope', NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE],
  ['declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly', false],
  ['declaredBoundedInterpolationUniqueIntervalBoundCreaseRowsParserOnly', true],
  ['declaredBoundedInterpolationAngleKnotCardinalityParserOnly', false],
  ['declaredBoundedInterpolationBoundKnotIntervalCardinalityParserOnly', false],
  ['declaredBoundedInterpolationAngleContainmentParserOnly', false],
  ['declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly', false],
  ['canonicalManifestPath', NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['representationSelectionEstablished', false],
...
function addLedgerIssue(
  issues: NegPathDuplicateIntervalBoundCreaseCandidateLedgerIssueV1[],
  path: string,
  code: NegPathDuplicateIntervalBoundCreaseCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathDuplicateIntervalBoundCreaseCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathDuplicateIntervalBoundCreaseCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [
      index,
      expected,
    ] of NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1,
  });
}
...
export function duplicateIntervalBoundCreaseJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...duplicateIntervalBoundCreaseJsonDifferencePaths(left[index], right[index], childPath),
        );
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      if (!Object.hasOwn(left, key) || !Object.hasOwn(right, key)) differences.push(childPath);
        ...
        differences.push(
          ...duplicateIntervalBoundCreaseJsonDifferencePaths(left[key], right[key], childPath),
        );
    }
    return differences;
  }
...
export type NegPathDuplicateIntervalBoundCreaseCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathDuplicateIntervalBoundCreaseCandidateBuildV1 = Readonly<{
  ledger: NegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1;
  files: readonly NegPathDuplicateIntervalBoundCreaseCandidateBuiltFileV1[];
}>;

export function buildNegPathDuplicateIntervalBoundCreaseCandidateBundleV1(): NegPathDuplicateIntervalBoundCreaseCandidateBuildV1 {
  if (
    !parseArtifactContractV1(NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_FOLD_CONTROL_SOURCE_V1).ok
  ) {
    throw new TypeError('accepted duplicate-interval-bound control no longer parses');
  }
  const differences = duplicateIntervalBoundCreaseJsonDifferencePaths(
  ...
  if (
    !sameStableJson(differences, NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.changedPaths)
  ) {
    throw new TypeError('duplicate-interval-bound control row-addition delta changed');
  }
  ...
  if (
    ...
    throw new TypeError('duplicate-interval-bound parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_README_V1],
    [
      CONTROL_ARTIFACT_ID,
      jsonLine(NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_FOLD_CONTROL_SOURCE_V1),
    ],
    [
      SOURCE_ARTIFACT_ID,
      jsonLine(NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.sourceDocument),
    ],
  ]);
  const artifacts = NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing duplicate-interval-bound artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathDuplicateIntervalBoundCreaseCandidateBuiltFileV1[] = artifacts.map(
    (artifact) => {
      const text = textByArtifactId.get(artifact.artifactId);
      if (text === undefined)
        throw new TypeError(`missing duplicate-interval-bound artifact ${artifact.artifactId}`);
      return { artifactId: artifact.artifactId, path: artifact.path, text };
    },
  );
  files.push({
    artifactId: null,
    path: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathDuplicateIntervalBoundCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathDuplicateIntervalBoundCreaseCandidateBuildV1> {
  const built = buildNegPathDuplicateIntervalBoundCreaseCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path duplicate-interval-bound candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathDuplicateIntervalBoundCreaseCandidateReasonCodeV1 =
  (typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_REASON_CODES)[number];
...
export type NegPathDuplicateIntervalBoundCreaseCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-duplicate-interval-bound-crease-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE-V1';
  scope: typeof NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathDuplicateIntervalBoundCreaseCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathDuplicateIntervalBoundCreaseCandidateReasonCodeV1>;
...
): NegPathDuplicateIntervalBoundCreaseCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_REASON_CODES.filter(
    (code) => state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType:
      'm0f-neg-path-duplicate-interval-bound-crease-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-INTERVAL-BOUND-CREASE-V1' as const,
    scope: NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_SCOPE,
    declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly: false as const,
    declaredBoundedInterpolationUniqueIntervalBoundCreaseRowsParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(
          resolve(NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANONICAL_MANIFEST_PATH),
          'utf8',
        ),
      ) as unknown,
    );
    ...
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathDuplicateIntervalBoundCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathDuplicateIntervalBoundCreaseCandidateVerificationResultV1> {
  ...
  try {
    ...
    const parsedLedger =
      parseNegPathDuplicateIntervalBoundCreaseCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          duplicateIntervalBoundCreaseJsonDifferencePaths(control, source),
          NEG_PATH_DUPLICATE_INTERVAL_BOUND_CREASE_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathDuplicateIntervalBoundCreaseCandidateBundleV1();
