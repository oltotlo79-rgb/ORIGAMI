...
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-empty-segments-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-empty-segments-candidate-bundle-ledger' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-path-candidate-non-empty-segments-parser-replay-only' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-empty-segments-candidate-bundle' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-EMPTY-SEGMENTS' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_EMPTY_SEGMENTS_CASE_ID = 'NEG-PATH-MUTATION-EMPTY-SEGMENTS' as const;

const SOURCE_REFERENCE_ID = 'oridesign-neg-path-empty-segments-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-empty-segments' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathEmptySegmentsSourceDocumentV1 = NegPathAngleBoundSourceDocumentV1;

function ownedAcceptedControl(): NegPathEmptySegmentsSourceDocumentV1 {
...
export const NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(ownedAcceptedControl());

function emptySegmentsSource(): NegPathEmptySegmentsSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1);
  const removed = source.pathCandidate.segments.splice(0, 1);
  if (
    removed.length !== 1 ||
    removed[0]?.t0 !== 0 ||
    removed[0].t1 !== 1 ||
    source.pathCandidate.segments.length !== 0
  ) {
    throw new TypeError('fixed sole path segment deletion drifted');
  }
  return source;
}
...
export const NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_EMPTY_SEGMENTS_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'delete-sole-path-candidate-segment',
  changedPaths: ['$.pathCandidate.segments[0]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-empty-segments.json',
  sourceDocument: emptySegmentsSource(),
  expectedIssues: [
    {
...
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-EMPTY-SEGMENTS exact-negative candidate vector
...
export type NegPathEmptySegmentsArtifactProvenanceV1 = Readonly<{
...
export type NegPathEmptySegmentsArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathEmptySegmentsArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathEmptySegmentsArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathEmptySegmentsArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_EMPTY_SEGMENTS_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathEmptySegmentsCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_EMPTY_SEGMENTS_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0]'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-EMPTY-SEGMENTS project-authored path-candidate non-empty-segments parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathEmptySegmentsCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_EMPTY_SEGMENTS_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathEmptySegmentsCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-EMPTY-SEGMENTS-V1';
  scope: typeof NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCOPE;
  declaredPathCandidateNonEmptySegmentsParserOnly: true;
  ...
  canonicalManifestPath: typeof NEG_PATH_EMPTY_SEGMENTS_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathEmptySegmentsCaseLedgerRowV1[];
  artifacts: readonly NegPathEmptySegmentsArtifactLedgerEntryV1[];
}>;

export type NegPathEmptySegmentsCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathEmptySegmentsCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathEmptySegmentsCandidateBundleLedgerV1 }>
  | Readonly<{
      ok: false;
      error: readonly NegPathEmptySegmentsCandidateLedgerIssueV1[];
    }>;
...
const ROOT_KEYS = [
  ...
  'vectorSetId',
  'scope',
  'declaredPathCandidateNonEmptySegmentsParserOnly',
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-EMPTY-SEGMENTS-V1'],
  ['scope', NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCOPE],
  ['declaredPathCandidateNonEmptySegmentsParserOnly', true],
  ['declaredPathCandidateRepresentationStatusParserOnly', false],
  ['declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly', false],
  ['declaredBoundedInterpolationUniqueIntervalBoundCreaseRowsParserOnly', false],
  ...
  ['declaredBoundedInterpolationAngleContainmentParserOnly', false],
  ['declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly', false],
  ['canonicalManifestPath', NEG_PATH_EMPTY_SEGMENTS_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['representationSelectionEstablished', false],
...
function addLedgerIssue(
  issues: NegPathEmptySegmentsCandidateLedgerIssueV1[],
  path: string,
  code: NegPathEmptySegmentsCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathEmptySegmentsCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathEmptySegmentsCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathEmptySegmentsCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.cases) || value.cases.length !== 1) {
    ...
    addLedgerIssue(
      ...
      '$.cases[0]',
      'invalid-case',
      'case identity, control link, empty-segments delta, path, or complete issue oracle differs',
    );
  }
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [index, expected] of NEG_PATH_EMPTY_SEGMENTS_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathEmptySegmentsCandidateBundleLedgerV1,
  });
}
...
export function emptySegmentsJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(...emptySegmentsJsonDifferencePaths(left[index], right[index], childPath));
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      else differences.push(...emptySegmentsJsonDifferencePaths(left[key], right[key], childPath));
    }
    return differences;
  }
  return [path];
...
export type NegPathEmptySegmentsCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathEmptySegmentsCandidateBuildV1 = Readonly<{
  ledger: NegPathEmptySegmentsCandidateBundleLedgerV1;
  files: readonly NegPathEmptySegmentsCandidateBuiltFileV1[];
}>;

export function buildNegPathEmptySegmentsCandidateBundleV1(): NegPathEmptySegmentsCandidateBuildV1 {
  if (!parseArtifactContractV1(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1).ok) {
    throw new TypeError('accepted non-empty-segments control no longer parses');
  }
  const differences = emptySegmentsJsonDifferencePaths(
    NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1,
    NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument,
  );
  if (!sameStableJson(differences, NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths)) {
    throw new TypeError('empty-segments deletion delta changed');
  }
  const replay = parseArtifactContractV1(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument);
  if (
    replay.ok ||
    !sameStableJson(
      issueSignature(replay.error),
      NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.expectedIssues,
    )
  ) {
    throw new TypeError('empty-segments parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_README_V1],
    [CONTROL_ARTIFACT_ID, jsonLine(NEG_PATH_EMPTY_SEGMENTS_FOLD_CONTROL_SOURCE_V1)],
    [SOURCE_ARTIFACT_ID, jsonLine(NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.sourceDocument)],
  ]);
  const artifacts = NEG_PATH_EMPTY_SEGMENTS_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing empty-segments artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathEmptySegmentsCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathEmptySegmentsCandidateBuiltFileV1[] = artifacts.map((artifact) => {
    const text = textByArtifactId.get(artifact.artifactId);
    if (text === undefined)
      throw new TypeError(`missing empty-segments artifact ${artifact.artifactId}`);
    return { artifactId: artifact.artifactId, path: artifact.path, text };
  });
  files.push({
    artifactId: null,
    path: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathEmptySegmentsCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathEmptySegmentsCandidateBuildV1> {
  const built = buildNegPathEmptySegmentsCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path empty-segments candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathEmptySegmentsCandidateReasonCodeV1 =
  (typeof NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_REASON_CODES)[number];
...
export type NegPathEmptySegmentsCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-empty-segments-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-EMPTY-SEGMENTS-V1';
  scope: typeof NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathEmptySegmentsCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathEmptySegmentsCandidateReasonCodeV1>;
...
): NegPathEmptySegmentsCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_REASON_CODES.filter((code) =>
    state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType: 'm0f-neg-path-empty-segments-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-EMPTY-SEGMENTS-V1' as const,
    scope: NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_SCOPE,
    declaredPathCandidateNonEmptySegmentsParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(resolve(NEG_PATH_EMPTY_SEGMENTS_CANONICAL_MANIFEST_PATH), 'utf8'),
      ) as unknown,
    );
    if (parsed.manifest === undefined) return undefined;
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_EMPTY_SEGMENTS_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathEmptySegmentsCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathEmptySegmentsCandidateVerificationResultV1> {
  ...
  try {
    ...
    try {
      ledgerText = await readFile(
        resolve(absoluteDirectory, NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME),
        'utf8',
      );
    ...
    const parsedLedger = parseNegPathEmptySegmentsCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const expectedNames = new Set([
      NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_EMPTY_SEGMENTS_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          emptySegmentsJsonDifferencePaths(control, source),
          NEG_PATH_EMPTY_SEGMENTS_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathEmptySegmentsCandidateBundleV1();
