...
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-non-monotonic-knot-time-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-non-monotonic-knot-time-candidate-bundle-ledger' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-bounded-interpolation-knot-time-strict-monotonicity-parser-replay-only' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-non-monotonic-knot-time-candidate-bundle' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_ID =
  'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME' as const;

const SOURCE_REFERENCE_ID = 'oridesign-neg-path-non-monotonic-knot-time-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-non-monotonic-knot-time' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathNonMonotonicKnotTimeSourceDocumentV1 = NegPathAngleBoundSourceDocumentV1;

function ownedAcceptedControl(): NegPathNonMonotonicKnotTimeSourceDocumentV1 {
  const control = structuredClone(NEG_PATH_ANGLE_BOUND_FOLD_CONTROL_SOURCE_V1);
  if (!parseArtifactContractV1(control).ok) {
    throw new TypeError('fixed bounded-interpolation knot control must be accepted');
  }
  return control;
}

export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(ownedAcceptedControl());

function nonMonotonicKnotTimeSource(): NegPathNonMonotonicKnotTimeSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_NON_MONOTONIC_KNOT_TIME_FOLD_CONTROL_SOURCE_V1);
  ...
  const motion = firstSegment.motion;
  const firstAngles = motion.anglesByCrease[0];
  const firstBounds = motion.intervalAngleBoundsByCrease[0];
  const firstIntervalBound = firstBounds?.bounds[0];
  const secondIntervalBound = firstBounds?.bounds[1];
  if (
    motion.knotTimes.length !== 3 ||
    motion.knotTimes[0] !== 0 ||
    motion.knotTimes[1] !== 0.5 ||
    motion.knotTimes[2] !== 1 ||
    firstAngles?.edgeId !== 'e-hinge' ||
    firstAngles.angles[0] !== 0 ||
    firstAngles.angles[1] !== Math.PI / 2 ||
    firstAngles.angles[2] !== Math.PI ||
    firstBounds?.edgeId !== 'e-hinge' ||
    firstIntervalBound?.[0] !== 0 ||
    firstIntervalBound[1] !== Math.PI / 2 ||
    secondIntervalBound?.[0] !== Math.PI / 2 ||
    secondIntervalBound[1] !== Math.PI
  ) {
    throw new TypeError('fixed three-knot bounded control drifted');
  }
  motion.knotTimes[1] = 0;
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'replace-middle-bounded-knot-time-with-segment-start',
  changedPaths: ['$.pathCandidate.segments[0].motion.knotTimes[1]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-non-monotonic-knot-time.json',
  sourceDocument: nonMonotonicKnotTimeSource(),
  expectedIssues: [
    {
      path: '$.pathCandidate.segments[0].motion.knotTimes',
      code: 'non-monotonic-time',
    },
  ],
} as const);

export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME exact-negative candidate vector
...
export type NegPathNonMonotonicKnotTimeArtifactProvenanceV1 = Readonly<{
...
export type NegPathNonMonotonicKnotTimeArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathNonMonotonicKnotTimeArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathNonMonotonicKnotTimeArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathNonMonotonicKnotTimeArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathNonMonotonicKnotTimeCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0].motion.knotTimes[1]'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME project-authored bounded-interpolation strict knot-time ordering parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathNonMonotonicKnotTimeCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathNonMonotonicKnotTimeCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME-V1';
  scope: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCOPE;
  declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly: true;
  canonicalManifestPath: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathNonMonotonicKnotTimeCaseLedgerRowV1[];
  artifacts: readonly NegPathNonMonotonicKnotTimeArtifactLedgerEntryV1[];
}>;

export type NegPathNonMonotonicKnotTimeCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathNonMonotonicKnotTimeCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathNonMonotonicKnotTimeCandidateBundleLedgerV1 }>
  | Readonly<{ ok: false; error: readonly NegPathNonMonotonicKnotTimeCandidateLedgerIssueV1[] }>;
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME-V1'],
  ['scope', NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCOPE],
  ['declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly', true],
  ['canonicalManifestPath', NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['angleKnotCardinalityEstablished', false],
...
function addLedgerIssue(
  issues: NegPathNonMonotonicKnotTimeCandidateLedgerIssueV1[],
  path: string,
  code: NegPathNonMonotonicKnotTimeCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathNonMonotonicKnotTimeCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathNonMonotonicKnotTimeCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathNonMonotonicKnotTimeCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.cases) || value.cases.length !== 1) {
    ...
    addLedgerIssue(
      ...
      '$.cases[0]',
      'invalid-case',
      'case identity, control link, middle-knot replacement delta, path, or complete issue oracle differs',
    );
  }
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [index, expected] of NEG_PATH_NON_MONOTONIC_KNOT_TIME_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathNonMonotonicKnotTimeCandidateBundleLedgerV1,
  });
}
...
export function nonMonotonicKnotTimeJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...nonMonotonicKnotTimeJsonDifferencePaths(left[index], right[index], childPath),
        );
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      if (!Object.hasOwn(left, key) || !Object.hasOwn(right, key)) differences.push(childPath);
        ...
        differences.push(
          ...nonMonotonicKnotTimeJsonDifferencePaths(left[key], right[key], childPath),
        );
    }
    return differences;
  }
...
export type NegPathNonMonotonicKnotTimeCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathNonMonotonicKnotTimeCandidateBuildV1 = Readonly<{
  ledger: NegPathNonMonotonicKnotTimeCandidateBundleLedgerV1;
  files: readonly NegPathNonMonotonicKnotTimeCandidateBuiltFileV1[];
}>;

export function buildNegPathNonMonotonicKnotTimeCandidateBundleV1(): NegPathNonMonotonicKnotTimeCandidateBuildV1 {
  if (!parseArtifactContractV1(NEG_PATH_NON_MONOTONIC_KNOT_TIME_FOLD_CONTROL_SOURCE_V1).ok) {
    throw new TypeError('accepted strict knot-time control no longer parses');
  }
  const differences = nonMonotonicKnotTimeJsonDifferencePaths(
    NEG_PATH_NON_MONOTONIC_KNOT_TIME_FOLD_CONTROL_SOURCE_V1,
    NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.sourceDocument,
  );
  if (!sameStableJson(differences, NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.changedPaths)) {
    throw new TypeError('strict knot-time replacement delta changed');
  }
  ...
  if (
    ...
    throw new TypeError('strict knot-time parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_README_V1],
    [CONTROL_ARTIFACT_ID, jsonLine(NEG_PATH_NON_MONOTONIC_KNOT_TIME_FOLD_CONTROL_SOURCE_V1)],
    [SOURCE_ARTIFACT_ID, jsonLine(NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.sourceDocument)],
  ]);
  const artifacts = NEG_PATH_NON_MONOTONIC_KNOT_TIME_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing strict knot-time artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathNonMonotonicKnotTimeCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathNonMonotonicKnotTimeCandidateBuiltFileV1[] = artifacts.map((artifact) => {
    const text = textByArtifactId.get(artifact.artifactId);
    if (text === undefined)
      throw new TypeError(`missing strict knot-time artifact ${artifact.artifactId}`);
    return { artifactId: artifact.artifactId, path: artifact.path, text };
  });
  files.push({
    artifactId: null,
    path: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathNonMonotonicKnotTimeCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathNonMonotonicKnotTimeCandidateBuildV1> {
  const built = buildNegPathNonMonotonicKnotTimeCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path strict knot-time candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathNonMonotonicKnotTimeCandidateReasonCodeV1 =
  (typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_REASON_CODES)[number];
...
export type NegPathNonMonotonicKnotTimeCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-non-monotonic-knot-time-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME-V1';
  scope: typeof NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathNonMonotonicKnotTimeCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathNonMonotonicKnotTimeCandidateReasonCodeV1>;
...
): NegPathNonMonotonicKnotTimeCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_REASON_CODES.filter((code) =>
    state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType:
      'm0f-neg-path-non-monotonic-knot-time-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-NON-MONOTONIC-KNOT-TIME-V1' as const,
    scope: NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_SCOPE,
    declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly: true as const,
    ...
    physicalTimeParameterizationEstablished: false as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(resolve(NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANONICAL_MANIFEST_PATH), 'utf8'),
      ) as unknown,
    );
    if (parsed.manifest === undefined) return undefined;
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathNonMonotonicKnotTimeCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathNonMonotonicKnotTimeCandidateVerificationResultV1> {
  ...
  try {
    ...
    const parsedLedger = parseNegPathNonMonotonicKnotTimeCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_NON_MONOTONIC_KNOT_TIME_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          nonMonotonicKnotTimeJsonDifferencePaths(control, source),
          NEG_PATH_NON_MONOTONIC_KNOT_TIME_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathNonMonotonicKnotTimeCandidateBundleV1();
