...
export async function runNegPathRepresentationStatusEscalationCandidateBundleCli(
  ...
  const actualIo =
    ...
    } satisfies NegPathRepresentationStatusEscalationCandidateBundleCliIo);
  ...
  const directory = resolve(
    actualIo.cwd,
    positional[0] ?? NEG_PATH_REPRESENTATION_STATUS_ESCALATION_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  );
  try {
    if (actions[0] === '--write')
    ...
    if (argv.includes('--json')) actualIo.stdout(`${stableStringify(result)}\n`);
      ...
      actualIo.stdout(
        'NEG-PATH-MUTATION-REPRESENTATION-STATUS-ESCALATION REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE (declared path-candidate representation-status parser replay only; no canonical promotion; no scientific claim; M0F GO not evaluated)\n',
      );
    } else {
    ...
    actualIo.stderr(
      'NEG-PATH-MUTATION-REPRESENTATION-STATUS-ESCALATION CANDIDATE BUNDLE GENERATION FAILED\n',
    );
    return 1;
  }
}
...
if (invokedPath !== undefined && import.meta.url === pathToFileURL(resolve(invokedPath)).href) {
  process.exitCode = await runNegPathRepresentationStatusEscalationCandidateBundleCli(
