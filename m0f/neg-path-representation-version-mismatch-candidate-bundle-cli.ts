...
} from './neg-path-representation-version-mismatch-candidate-bundle.js';
import { stableStringify } from './stable-json.js';

export type NegPathRepresentationVersionMismatchCandidateBundleCliIo = Readonly<{
  cwd: string;
  stdout: (text: string) => void;
  stderr: (text: string) => void;
}>;

const USAGE = `Usage: neg-path-representation-version-mismatch-candidate-bundle [--verify|--write] [directory] [--json]

Replays or regenerates only one project-authored path-candidate representation-version mismatch rejection.
...
export async function runNegPathRepresentationVersionMismatchCandidateBundleCli(
  argv: readonly string[],
  io?: NegPathRepresentationVersionMismatchCandidateBundleCliIo,
): Promise<number> {
  const actualIo =
    io ??
    ({
    ...
    } satisfies NegPathRepresentationVersionMismatchCandidateBundleCliIo);
  ...
  const directory = resolve(
    actualIo.cwd,
    positional[0] ?? NEG_PATH_REPRESENTATION_VERSION_MISMATCH_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
  );
  try {
    if (actions[0] === '--write')
    ...
    if (argv.includes('--json')) actualIo.stdout(`${stableStringify(result)}\n`);
      ...
      actualIo.stdout(
        'NEG-PATH-MUTATION-REPRESENTATION-VERSION-MISMATCH REPRODUCIBLE EXACT-NEGATIVE CANDIDATE BUNDLE (declared path-candidate representation-version parser replay only; no canonical promotion; no scientific claim; M0F GO not evaluated)\n',
      );
    } else {
    ...
    actualIo.stderr(
      'NEG-PATH-MUTATION-REPRESENTATION-VERSION-MISMATCH CANDIDATE BUNDLE GENERATION FAILED\n',
    );
    return 1;
  }
}
...
if (invokedPath !== undefined && import.meta.url === pathToFileURL(resolve(invokedPath)).href) {
  process.exitCode = await runNegPathRepresentationVersionMismatchCandidateBundleCli(
