...
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-unsupported-representation-kind-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-unsupported-representation-kind-candidate-bundle-ledger' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-path-candidate-supported-representation-kind-parser-replay-only' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-unsupported-representation-kind-candidate-bundle' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_ID =
  'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND' as const;

const SOURCE_REFERENCE_ID =
  'oridesign-neg-path-unsupported-representation-kind-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-unsupported-representation-kind' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathUnsupportedRepresentationKindSourceDocumentV1 =
  NegPathAngleBoundSourceDocumentV1;

function ownedAcceptedControl(): NegPathUnsupportedRepresentationKindSourceDocumentV1 {
...
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(ownedAcceptedControl());

function unsupportedRepresentationKindSource(): NegPathUnsupportedRepresentationKindSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1);
  const segment = source.pathCandidate.segments[0];
  if (
    source.pathCandidate.segments.length !== 1 ||
    segment?.motion.kind !== 'bounded-interpolation'
  ) {
    throw new TypeError('fixed supported representation kind is missing before replacement');
  }
  (segment.motion as unknown as Record<string, unknown>).kind = 'spline';
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'replace-supported-path-representation-kind-with-spline',
  changedPaths: ['$.pathCandidate.segments[0].motion.kind'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-unsupported-representation-kind.json',
  sourceDocument: unsupportedRepresentationKindSource(),
  expectedIssues: [
    {
      path: '$.pathCandidate.segments[0].motion.kind',
      code: 'invalid-enum',
    },
  ],
} as const);

export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND exact-negative candidate vector

This closed directory contains one saved project-authored single-segment bounded-interpolation FOLD control and one exact-negative mutation for the current \`parseArtifactContractV1\` supported path-representation-kind boundary. The accepted owned control declares representation version 1, status \`candidate\`, knot times \`[0,0.5,1]\`, one valid \`e-hinge\` angle row \`[0,pi/2,pi]\`, and matching interval bounds \`[[0,pi/2],[pi/2,pi]]\`. The mutation changes only the segment motion \`kind\` from \`bounded-interpolation\` to the unsupported string \`spline\`. It fixes the sole \`invalid-enum\` issue with message \`unsupported path representation\` at that leaf and no secondary issue.

The bundle is intentionally outside \`tests/fixtures/manifest.json\`; \`NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND\` is not canonically registered or promoted. Exact-negative means only that the current artifact-contract parser rejects these saved bytes with the complete ordered issue signature fixed in the ledger.

This replay establishes only rejection of the saved unsupported path-representation-kind string. It does not establish representation-status handling, bounded-interpolation angle- or interval-bound-row uniqueness, angle/knot or bound/knot-interval cardinality, motion- or crease-map completeness, angle containment, strict knot ordering, path-time coverage, endpoint or physical continuity, representation selection or completeness, polynomial semantics, a radian convention, physical angle schedules or bounds, conservative bounds, kinematic feasibility, rigidity, face isometry, hinge geometry, contact analysis, CCD, collision detection or freedom, foldability, certificate-hash verification, cryptographic authenticity, canonical path-mutation-family completeness, SupportProfile, ToleranceProfile, scientific verification, or M0F GO. Local SHA-256 rows detect saved-byte drift only and are not signatures.

Verify with \`npx tsx m0f/neg-path-unsupported-representation-kind-candidate-bundle-cli.ts --verify\`. Regenerate with \`npx tsx m0f/neg-path-unsupported-representation-kind-candidate-bundle-cli.ts --write\`.
`;

export type NegPathUnsupportedRepresentationKindArtifactProvenanceV1 = Readonly<{
...
export type NegPathUnsupportedRepresentationKindArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathUnsupportedRepresentationKindArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathUnsupportedRepresentationKindArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathUnsupportedRepresentationKindArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathUnsupportedRepresentationKindCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0].motion.kind'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND project-authored supported path-representation-kind parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathUnsupportedRepresentationKindCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathUnsupportedRepresentationKindCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND-V1';
  scope: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCOPE;
  declaredPathCandidateSupportedRepresentationKindParserOnly: true;
  ...
  canonicalManifestPath: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathUnsupportedRepresentationKindCaseLedgerRowV1[];
  artifacts: readonly NegPathUnsupportedRepresentationKindArtifactLedgerEntryV1[];
}>;

export type NegPathUnsupportedRepresentationKindCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathUnsupportedRepresentationKindCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathUnsupportedRepresentationKindCandidateBundleLedgerV1 }>
  | Readonly<{
      ok: false;
      error: readonly NegPathUnsupportedRepresentationKindCandidateLedgerIssueV1[];
    }>;
const ROOT_KEYS = [
  'schemaVersion',
  'schemaId',
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND-V1'],
  ['scope', NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCOPE],
  ['declaredPathCandidateSupportedRepresentationKindParserOnly', true],
  ['declaredPathCandidateRepresentationStatusParserOnly', false],
  ['declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly', false],
  ['declaredBoundedInterpolationUniqueIntervalBoundCreaseRowsParserOnly', false],
  ...
  ['declaredBoundedInterpolationAngleContainmentParserOnly', false],
  ['declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly', false],
  ['canonicalManifestPath', NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['representationSelectionEstablished', false],
...
function addLedgerIssue(
  issues: NegPathUnsupportedRepresentationKindCandidateLedgerIssueV1[],
  path: string,
  code: NegPathUnsupportedRepresentationKindCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathUnsupportedRepresentationKindCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathUnsupportedRepresentationKindCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [
      index,
      expected,
    ] of NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathUnsupportedRepresentationKindCandidateBundleLedgerV1,
  });
}
...
export function unsupportedRepresentationKindJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...unsupportedRepresentationKindJsonDifferencePaths(left[index], right[index], childPath),
        );
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      if (!Object.hasOwn(left, key) || !Object.hasOwn(right, key)) differences.push(childPath);
        ...
        differences.push(
          ...unsupportedRepresentationKindJsonDifferencePaths(left[key], right[key], childPath),
        );
    }
    return differences;
  }
...
export type NegPathUnsupportedRepresentationKindCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathUnsupportedRepresentationKindCandidateBuildV1 = Readonly<{
  ledger: NegPathUnsupportedRepresentationKindCandidateBundleLedgerV1;
  files: readonly NegPathUnsupportedRepresentationKindCandidateBuiltFileV1[];
}>;

export function buildNegPathUnsupportedRepresentationKindCandidateBundleV1(): NegPathUnsupportedRepresentationKindCandidateBuildV1 {
  if (
    !parseArtifactContractV1(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1).ok
  ) {
    throw new TypeError('accepted supported-representation-kind control no longer parses');
  }
  const differences = unsupportedRepresentationKindJsonDifferencePaths(
    NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1,
    NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourceDocument,
  );
  if (
    !sameStableJson(differences, NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.changedPaths)
  ) {
    throw new TypeError('unsupported-representation-kind delta changed');
  }
  ...
  if (
    ...
    !sameStableJson(
      ...
      NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.expectedIssues,
    )
  ) {
    throw new TypeError('unsupported-representation-kind parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_README_V1],
    [
      CONTROL_ARTIFACT_ID,
      jsonLine(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_FOLD_CONTROL_SOURCE_V1),
    ],
    [
      SOURCE_ARTIFACT_ID,
      jsonLine(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.sourceDocument),
    ],
  ]);
  const artifacts = NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing unsupported-representation-kind artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathUnsupportedRepresentationKindCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathUnsupportedRepresentationKindCandidateBuiltFileV1[] = artifacts.map(
    (artifact) => {
      const text = textByArtifactId.get(artifact.artifactId);
      if (text === undefined)
        throw new TypeError(
          `missing unsupported-representation-kind artifact ${artifact.artifactId}`,
        );
  ...
  files.push({
    artifactId: null,
    path: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathUnsupportedRepresentationKindCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathUnsupportedRepresentationKindCandidateBuildV1> {
  const built = buildNegPathUnsupportedRepresentationKindCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path unsupported-representation-kind candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathUnsupportedRepresentationKindCandidateReasonCodeV1 =
  (typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_REASON_CODES)[number];
...
export type NegPathUnsupportedRepresentationKindCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-unsupported-representation-kind-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND-V1';
  scope: typeof NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathUnsupportedRepresentationKindCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathUnsupportedRepresentationKindCandidateReasonCodeV1>;
...
): NegPathUnsupportedRepresentationKindCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_REASON_CODES.filter(
    (code) => state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType:
      'm0f-neg-path-unsupported-representation-kind-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-UNSUPPORTED-REPRESENTATION-KIND-V1' as const,
    scope: NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_SCOPE,
    declaredPathCandidateSupportedRepresentationKindParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(
          resolve(NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANONICAL_MANIFEST_PATH),
          'utf8',
        ),
      ) as unknown,
    );
    ...
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathUnsupportedRepresentationKindCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathUnsupportedRepresentationKindCandidateVerificationResultV1> {
  ...
  try {
    ...
    try {
      ledgerText = await readFile(
        resolve(
          absoluteDirectory,
          NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_LEDGER_FILENAME,
        ),
        'utf8',
      );
    ...
    const parsedLedger =
      parseNegPathUnsupportedRepresentationKindCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          unsupportedRepresentationKindJsonDifferencePaths(control, source),
          NEG_PATH_UNSUPPORTED_REPRESENTATION_KIND_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathUnsupportedRepresentationKindCandidateBundleV1();
