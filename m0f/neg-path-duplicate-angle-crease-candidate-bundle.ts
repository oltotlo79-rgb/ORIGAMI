...
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID =
  'https://oridesign.local/schemas/m0f/neg-path-duplicate-angle-crease-candidate-bundle-ledger-v1.schema.json' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE =
  'm0f-neg-path-duplicate-angle-crease-candidate-bundle-ledger' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCOPE =
  'project-authored-exact-negative-artifact-contract-bounded-interpolation-unique-angle-crease-rows-parser-replay-only' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID =
  'oridesign-neg-path-duplicate-angle-crease-candidate-bundle' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION =
  '1.0.0-candidate' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY =
  'tests/candidate-vectors/NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME =
  'candidate-bundle-ledger.json' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANONICAL_MANIFEST_PATH =
  'tests/fixtures/manifest.json' as const;
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_ID =
  'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE' as const;

const SOURCE_REFERENCE_ID = 'oridesign-neg-path-duplicate-angle-crease-candidate-v1' as const;
const CONTROL_ARTIFACT_ID = 'control-fold' as const;
const SOURCE_ARTIFACT_ID = 'source-path-duplicate-angle-crease' as const;
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/;

export type NegPathDuplicateAngleCreaseSourceDocumentV1 = NegPathAngleBoundSourceDocumentV1;

function ownedAcceptedControl(): NegPathDuplicateAngleCreaseSourceDocumentV1 {
...
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1 =
  deepFreezeOwned(ownedAcceptedControl());

function duplicateAngleCreaseSource(): NegPathDuplicateAngleCreaseSourceDocumentV1 {
  const source = structuredClone(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1);
  ...
  if (
    firstSegment.motion.anglesByCrease.length !== 1 ||
    firstAngles?.edgeId !== 'e-hinge' ||
    firstAngles.angles[0] !== 0 ||
    firstAngles.angles[1] !== Math.PI / 2 ||
    firstAngles.angles[2] !== Math.PI
  ) {
    throw new TypeError('fixed valid angle row is missing');
  }
  firstSegment.motion.anglesByCrease.push(structuredClone(firstAngles));
  return source;
}

type ExpectedIssueV1 = Readonly<{ path: string; code: string }>;

export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1 = deepFreezeOwned({
  caseId: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_ID,
  controlArtifactId: CONTROL_ARTIFACT_ID,
  mutationKind: 'append-duplicate-bounded-interpolation-angle-crease-row',
  changedPaths: ['$.pathCandidate.segments[0].motion.anglesByCrease[1]'],
  sourceArtifactId: SOURCE_ARTIFACT_ID,
  sourcePath: 'path-duplicate-angle-crease.json',
  sourceDocument: duplicateAngleCreaseSource(),
  expectedIssues: [
    {
      path: '$.pathCandidate.segments[0].motion.anglesByCrease[1].edgeId',
      code: 'duplicate-reference',
    },
  ],
} as const);

export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_README_V1 = `# NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE exact-negative candidate vector

This closed directory contains one saved project-authored single-segment bounded-interpolation FOLD control and one exact-negative mutation for the current \`parseArtifactContractV1\` unique angle crease-row check. The accepted owned control declares knot times \`[0,0.5,1]\`, one valid \`e-hinge\` angle row \`[0,pi/2,pi]\`, and matching interval bounds \`[[0,pi/2],[pi/2,pi]]\`. The mutation appends only a clone of the valid angle row at index 1. It fixes the sole \`duplicate-reference\` issue at the appended row's \`edgeId\`; the angle and interval-bound crease-reference sets remain equal, so there is no motion-map or other secondary issue.

The bundle is intentionally outside \`tests/fixtures/manifest.json\`; \`NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE\` is not canonically registered or promoted. Exact-negative means only that the current artifact-contract parser rejects these saved bytes with the complete ordered issue signature fixed in the ledger.

This replay establishes only rejection of the saved duplicate bounded-interpolation angle crease-row reference. It does not establish interval-bound row uniqueness, angle/knot or bound/knot-interval cardinality, motion- or crease-map completeness, angle containment, strict knot ordering, path-time coverage, endpoint or physical continuity, representation selection or completeness, polynomial semantics, a radian convention, physical angle schedules or bounds, conservative bounds, kinematic feasibility, rigidity, face isometry, hinge geometry, contact analysis, CCD, collision detection or freedom, foldability, certificate-hash verification, cryptographic authenticity, canonical path-mutation-family completeness, SupportProfile, ToleranceProfile, scientific verification, or M0F GO. Local SHA-256 rows detect saved-byte drift only and are not signatures.

Verify with \`npx tsx m0f/neg-path-duplicate-angle-crease-candidate-bundle-cli.ts --verify\`. Regenerate with \`npx tsx m0f/neg-path-duplicate-angle-crease-candidate-bundle-cli.ts --write\`.
`;

export type NegPathDuplicateAngleCreaseArtifactProvenanceV1 = Readonly<{
...
export type NegPathDuplicateAngleCreaseArtifactLedgerEntryV1 = Readonly<{
  artifactId: string;
  ...
  licenseSpdx: 'MIT';
  provenance: NegPathDuplicateAngleCreaseArtifactProvenanceV1;
}>;

function artifactProvenance(
  dependsOnArtifactIds: readonly (typeof CONTROL_ARTIFACT_ID)[],
): NegPathDuplicateAngleCreaseArtifactProvenanceV1 {
  return {
...
type ArtifactSpecV1 = Omit<NegPathDuplicateAngleCreaseArtifactLedgerEntryV1, 'sha256'>;

export const NEG_PATH_DUPLICATE_ANGLE_CREASE_ARTIFACT_SPECS_V1 = deepFreezeOwned([
  {
  ...
  },
  {
    ...
    path: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourcePath,
    mediaType: 'application/json',
    licenseSpdx: 'MIT',
...
export type NegPathDuplicateAngleCreaseCaseLedgerRowV1 = Readonly<{
  caseIndex: 0;
  caseId: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_ID;
  controlArtifactId: typeof CONTROL_ARTIFACT_ID;
  mutationKind: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.mutationKind;
  changedPaths: readonly ['$.pathCandidate.segments[0].motion.anglesByCrease[1]'];
...
const GENERATOR = deepFreezeOwned({
  generatorId: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_GENERATOR_ID,
  generatorVersion: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_GENERATOR_VERSION,
...
const SOURCE_PROVENANCE = deepFreezeOwned({
  ...
  title:
    'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE project-authored bounded-interpolation unique angle crease-row parser vector',
  authors: ['OriDesign contributors'],
  redistribution: 'allowed' as const,
  licenseSpdx: 'MIT' as const,
});

function ledgerCases(): readonly NegPathDuplicateAngleCreaseCaseLedgerRowV1[] {
  return [
    {
      caseIndex: 0,
      caseId: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_ID,
      controlArtifactId: CONTROL_ARTIFACT_ID,
      mutationKind: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.mutationKind,
      changedPaths: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.changedPaths,
      sourceArtifactId: SOURCE_ARTIFACT_ID,
      sourcePath: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourcePath,
      expectedOutcome: 'rejected',
      expectedIssues: NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.expectedIssues,
    },
  ];
}

export type NegPathDuplicateAngleCreaseCandidateBundleLedgerV1 = Readonly<{
  schemaVersion: 1;
  schemaId: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID;
  recordType: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE;
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE-V1';
  scope: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCOPE;
  declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly: true;
  ...
  canonicalManifestPath: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANONICAL_MANIFEST_PATH;
  ...
  cases: readonly NegPathDuplicateAngleCreaseCaseLedgerRowV1[];
  artifacts: readonly NegPathDuplicateAngleCreaseArtifactLedgerEntryV1[];
}>;

export type NegPathDuplicateAngleCreaseCandidateLedgerIssueV1 = Readonly<{
  path: string;
  code:
    | 'invalid-snapshot'
    | 'invalid-object'
...
export type NegPathDuplicateAngleCreaseCandidateLedgerParseResultV1 =
  | Readonly<{ ok: true; value: NegPathDuplicateAngleCreaseCandidateBundleLedgerV1 }>
  | Readonly<{ ok: false; error: readonly NegPathDuplicateAngleCreaseCandidateLedgerIssueV1[] }>;
...
const FIXED_LITERALS = [
  ['schemaVersion', 1],
  ['schemaId', NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID],
  ['recordType', NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE],
  ['contractStatus', 'candidate'],
  ['scientificClaim', false],
  ['vectorSetId', 'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE-V1'],
  ['scope', NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCOPE],
  ['declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly', true],
  ['declaredBoundedInterpolationUniqueIntervalBoundCreaseRowsParserOnly', false],
  ['declaredBoundedInterpolationAngleKnotCardinalityParserOnly', false],
  ['declaredBoundedInterpolationBoundKnotIntervalCardinalityParserOnly', false],
  ['declaredBoundedInterpolationAngleContainmentParserOnly', false],
  ['declaredBoundedInterpolationKnotTimeStrictMonotonicityParserOnly', false],
  ['canonicalManifestPath', NEG_PATH_DUPLICATE_ANGLE_CREASE_CANONICAL_MANIFEST_PATH],
  ['canonicalManifestRegistration', 'not-registered'],
  ['canonicalPromotionClaimed', false],
  ['canonicalPathMutationFamilyComplete', false],
  ['representationSelectionEstablished', false],
...
function addLedgerIssue(
  issues: NegPathDuplicateAngleCreaseCandidateLedgerIssueV1[],
  path: string,
  code: NegPathDuplicateAngleCreaseCandidateLedgerIssueV1['code'],
  message: string,
): void {
...
export function parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1(
  supplied: unknown,
): NegPathDuplicateAngleCreaseCandidateLedgerParseResultV1 {
  ...
  const issues: NegPathDuplicateAngleCreaseCandidateLedgerIssueV1[] = [];
  ...
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== 3) {
    ...
    for (const [index, expected] of NEG_PATH_DUPLICATE_ANGLE_CREASE_ARTIFACT_SPECS_V1.entries()) {
  ...
  return deepFreezeOwned({
    ok: true as const,
    value: value as unknown as NegPathDuplicateAngleCreaseCandidateBundleLedgerV1,
  });
}
...
export function duplicateAngleCreaseJsonDifferencePaths(
  left: unknown,
  right: unknown,
  path = '$',
): string[] {
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    for (let index = 0; index < Math.max(left.length, right.length); index += 1) {
      ...
      if (!Object.hasOwn(left, index) || !Object.hasOwn(right, index)) differences.push(childPath);
        ...
        differences.push(
          ...duplicateAngleCreaseJsonDifferencePaths(left[index], right[index], childPath),
        );
    }
    return differences;
  }
  if (isRecord(left) && isRecord(right)) {
    ...
    for (const key of keys) {
      ...
      if (!Object.hasOwn(left, key) || !Object.hasOwn(right, key)) differences.push(childPath);
        ...
        differences.push(
          ...duplicateAngleCreaseJsonDifferencePaths(left[key], right[key], childPath),
        );
    }
    return differences;
  }
...
export type NegPathDuplicateAngleCreaseCandidateBuiltFileV1 = Readonly<{
  artifactId: string | null;
  path: string;
  text: string;
}>;

export type NegPathDuplicateAngleCreaseCandidateBuildV1 = Readonly<{
  ledger: NegPathDuplicateAngleCreaseCandidateBundleLedgerV1;
  files: readonly NegPathDuplicateAngleCreaseCandidateBuiltFileV1[];
}>;

export function buildNegPathDuplicateAngleCreaseCandidateBundleV1(): NegPathDuplicateAngleCreaseCandidateBuildV1 {
  if (!parseArtifactContractV1(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1).ok) {
    throw new TypeError('accepted duplicate-angle control no longer parses');
  }
  const differences = duplicateAngleCreaseJsonDifferencePaths(
    NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1,
    NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourceDocument,
  );
  if (!sameStableJson(differences, NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.changedPaths)) {
    throw new TypeError('duplicate-angle control row-addition delta changed');
  }
  ...
  if (
    ...
    !sameStableJson(
      ...
      NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.expectedIssues,
    )
  ) {
    throw new TypeError('duplicate-angle parser oracle changed');
  }
  const textByArtifactId = new Map<string, string>([
    ['bundle-readme', NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_README_V1],
    [CONTROL_ARTIFACT_ID, jsonLine(NEG_PATH_DUPLICATE_ANGLE_CREASE_FOLD_CONTROL_SOURCE_V1)],
    [SOURCE_ARTIFACT_ID, jsonLine(NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.sourceDocument)],
  ]);
  const artifacts = NEG_PATH_DUPLICATE_ANGLE_CREASE_ARTIFACT_SPECS_V1.map((spec) => {
    const text = textByArtifactId.get(spec.artifactId);
    if (text === undefined)
      throw new TypeError(`missing duplicate-angle artifact ${spec.artifactId}`);
    return { ...spec, sha256: sha256Prefixed(text) as `sha256:${string}` };
  });
  const ledger: NegPathDuplicateAngleCreaseCandidateBundleLedgerV1 = {
    schemaVersion: 1,
    schemaId: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCHEMA_ID,
    recordType: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_RECORD_TYPE,
    contractStatus: 'candidate',
  ...
  const files: NegPathDuplicateAngleCreaseCandidateBuiltFileV1[] = artifacts.map((artifact) => {
    const text = textByArtifactId.get(artifact.artifactId);
    if (text === undefined)
      throw new TypeError(`missing duplicate-angle artifact ${artifact.artifactId}`);
    return { artifactId: artifact.artifactId, path: artifact.path, text };
  });
  files.push({
    artifactId: null,
    path: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
    text: jsonLine(ledger),
  });
  return deepFreezeOwned({ ledger, files });
}

export async function writeNegPathDuplicateAngleCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathDuplicateAngleCreaseCandidateBuildV1> {
  const built = buildNegPathDuplicateAngleCreaseCandidateBundleV1();
  ...
  if (existing.some((entry) => !entry.isFile() || !expectedNames.has(entry.name))) {
    throw new TypeError(
      'negative path duplicate-angle candidate directory contains an unexpected entry',
    );
  }
  await Promise.all(
...
export const NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_REASON_CODES = [
  'ledger-unreadable',
  'ledger-invalid',
...
export type NegPathDuplicateAngleCreaseCandidateReasonCodeV1 =
  (typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_REASON_CODES)[number];
...
export type NegPathDuplicateAngleCreaseCandidateVerificationResultV1 = Readonly<{
  schemaVersion: 1;
  recordType: 'm0f-neg-path-duplicate-angle-crease-candidate-bundle-verification-result';
  contractStatus: 'candidate';
  scientificClaim: false;
  vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE-V1';
  scope: typeof NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCOPE;
  ...
  reasonCodes: readonly NegPathDuplicateAngleCreaseCandidateReasonCodeV1[];
  checks: VerificationChecks;
}>;

interface MutableVerificationState {
  reasons: Set<NegPathDuplicateAngleCreaseCandidateReasonCodeV1>;
...
): NegPathDuplicateAngleCreaseCandidateVerificationResultV1 {
  const reasonCodes = NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_REASON_CODES.filter((code) =>
    state.reasons.has(code),
  );
  return deepFreezeOwned({
    schemaVersion: 1 as const,
    recordType: 'm0f-neg-path-duplicate-angle-crease-candidate-bundle-verification-result' as const,
    contractStatus: 'candidate' as const,
    scientificClaim: false as const,
    vectorSetId: 'NEG-PATH-MUTATION-DUPLICATE-ANGLE-CREASE-V1' as const,
    scope: NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_SCOPE,
    declaredBoundedInterpolationUniqueAngleCreaseRowsParserOnly: true as const,
...
async function canonicalCaseIsAbsent(): Promise<boolean | undefined> {
  try {
    const parsed = parseFixtureManifest(
      JSON.parse(
        await readFile(resolve(NEG_PATH_DUPLICATE_ANGLE_CREASE_CANONICAL_MANIFEST_PATH), 'utf8'),
      ) as unknown,
    );
    if (parsed.manifest === undefined) return undefined;
    return !parsed.manifest.fixtures.some(
      (fixture) => fixture.id === NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_ID,
    );
  } catch {
    return undefined;
  }
...
export async function verifyNegPathDuplicateAngleCreaseCandidateBundleV1(
  bundleDirectory: string = NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_DEFAULT_DIRECTORY,
): Promise<NegPathDuplicateAngleCreaseCandidateVerificationResultV1> {
  ...
  try {
    ...
    try {
      ledgerText = await readFile(
        resolve(
          absoluteDirectory,
          NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME,
        ),
        'utf8',
      );
    ...
    const parsedLedger = parseNegPathDuplicateAngleCreaseCandidateBundleLedgerV1(ledgerDocument);
    if (!parsedLedger.ok) {
    ...
    const textByPath = new Map<string, string>([
      [NEG_PATH_DUPLICATE_ANGLE_CREASE_CANDIDATE_BUNDLE_LEDGER_FILENAME, ledgerText],
    ]);
    ...
    if (source === undefined) {
      ...
      if (
        control !== undefined &&
        sameStableJson(
          duplicateAngleCreaseJsonDifferencePaths(control, source),
          NEG_PATH_DUPLICATE_ANGLE_CREASE_CASE_SPEC_V1.changedPaths,
        )
      ) {
    ...
    const regenerated = buildNegPathDuplicateAngleCreaseCandidateBundleV1();
